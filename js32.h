...
class JSAutoRoot {
 ...
 public:
  JSAutoRoot(JSContext* cx, jsval value) : cx(cx) {
    ref = new jsval(value);
    JS_AddValueRoot(cx, ref);
  }
  ~JSAutoRoot() {
...
template <class T>
struct Converter {
  jsval operator()(JSContext* cx, const jschar* c) {
    return STRING_TO_JSVAL(JS_NewUCStringCopyZ(cx, c));
  }
  jsval operator()(JSContext* cx, unsigned char c) {
    return UINT_TO_JSVAL(c);
  }
  jsval operator()(JSContext* cx, char* c) {
    return STRING_TO_JSVAL(JS_NewStringCopyZ(cx, c));
  }
  jsval operator()(JSContext* cx, int i) {
    return INT_TO_JSVAL(i);
  }
  jsval operator()(JSContext* cx, double d) {
    return DOUBLE_TO_JSVAL(d);
  }
  jsval operator()(JSContext* cx, uint32 ui) {
    return UINT_TO_JSVAL(ui);
  }
  jsval operator()(JSContext* cx, bool b) {
    return BOOLEAN_TO_JSVAL(b);
  }

  const jschar* operator()(JSContext* cx, jsval v, const jschar*) {
    return JS_GetStringCharsZ(cx, JSVAL_TO_STRING(v));
  }
  unsigned char operator()(JSContext* cx, jsval v, unsigned char) {
    return (unsigned char)JSVAL_TO_INT(v);
  }
  int operator()(JSContext* cx, jsval v, int) {
    return JSVAL_TO_INT(v);
  }
  double operator()(JSContext* cx, jsval v, double) {
    return JSVAL_TO_DOUBLE(v);
  }
  uint32 operator()(JSContext* cx, jsval v, uint32) {
    return (uint32)JSVAL_TO_INT(v);
  }
  bool operator()(JSContext* cx, jsval v, bool) {
