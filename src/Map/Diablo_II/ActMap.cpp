...
void ActMap::ClearCache(void) {
  ...
  for (ActMapList::iterator it = cache.begin(); it != end; it++) delete it->second;
  cache.clear();
...
void ActMap::DumpLevel(const char* file) const {
  ...
  for (D2DrlgRoomStrc* room = level->pRoom2First; room; room = room->pRoom2Next) {
    for (D2PresetUnitStrc* preset = room->pPreset; preset; preset = preset->pPresetNext) {
      ...
      fprintf(f, "Preset %d: at (%d, %d, relative: %d, %d) of type %d, txtFile %d, _1 %d, _3 %d, room (%d, %d) %s\n", i++, loc.first, loc.second, rel.first, rel.second,
              preset->dwType, preset->dwTxtFileNo, preset->_1, preset->_3, room->dwPosX, room->dwPosY, wName);
    }
  }
