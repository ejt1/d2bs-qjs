...
void LevelMap::ClearCache(void) {
  ...
  for (LevelMapList::iterator it = cache.begin(); it != end; it++) delete it->second;
  cache.clear();
}
    ...
    for (int j = x; j < _height && p != map->pMapEnd; j++, p++) {
      if ((*p & LevelMap::ClosedDoor) == LevelMap::ClosedDoor || (*p & LevelMap::Player) == LevelMap::Player) {
        SetCollisionData(j, i, 0);
                  ...
                  (*p & LevelMap::NPCLocation) == LevelMap::NPCLocation || (*p & LevelMap::DeadBody) == LevelMap::DeadBody || (*p & LevelMap::Item) == LevelMap::Item
...
bool LevelMap::IsGap(int x, int y, bool abs) const {
  ...
  for (int i = x - gapSize - 1; i <= x + gapSize + 1 && spaces < gapSize; i++) {
    Point _pt(i, y);
    if (IsValidPoint(_pt, abs))
      spaces = (SpaceIsWalkable(_pt, abs) ? 0 : spaces + 1);
  }
