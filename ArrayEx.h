...
int CArrayEx<TYPE, ARG_TYPE>::Find(ARG_TYPE tData, int nStartAt /*=0*/) const {
  ...
  if (m_nSort == AE_SORT_ASCENDING)  // ascending
  {
    ...
    while (nIndex > 0 && m_pData[nIndex - 1] == tData) nIndex--;
    return nIndex;
...
int CArrayEx<TYPE, ARG_TYPE>::ReverseFind(ARG_TYPE tData) const {
  ...
  if (m_nSort == AE_SORT_ASCENDING)  // ascending
  {
    ...
    int nIndex = (p == NULL) ? -1 : int(((long)p - (long)m_pData) / sizeof(TYPE));
    while (nIndex < m_nElementCount - 1 && m_pData[nIndex + 1] == tData) nIndex++;
    return nIndex;
  } else if (m_nSort == AE_SORT_DESCENDING)  // descending
  {
...
int CArrayEx<TYPE, ARG_TYPE>::InsertAt(int nIndex, ARG_TYPE tData, int nCount) {
  ...
  for (int i = 0; i < nCount; i++) m_pData[nIndex + i] = tData;
...
int CArrayEx<TYPE, ARG_TYPE>::InsertAt(int nIndex, const CArrayEx* pNewArray) {
  ...
  for (int i = 0; i < pNewArray->GetSize(); i++) m_pData[nIndex + i] = pNewArray->GetAt(i);
...
int CArrayEx<TYPE, ARG_TYPE>::RemoveAt(int nIndex, int nCount) {
  ...
  for (int i = 0; i < SEG; i++) m_pData[nIndex + i] = m_pData[nIndex + i + nCount];
...
BOOL CArrayEx<TYPE, ARG_TYPE>::FreeExtra() {
  ...
  for (int i = 0; i < m_nElementCount; i++) p[i] = m_pData[i];
...
BOOL CArrayEx<TYPE, ARG_TYPE>::Copy(const CArrayEx& src) {
  ...
  for (int i = 0; i < m_nElementCount; i++) m_pData[i] = src.m_pData[i];
  m_nSort = src.m_nSort;
  return TRUE;
}
...
int CArrayEx<TYPE, ARG_TYPE>::Append(const CArrayEx& src) {
  ...
  // append at the end
  for (int i = 0; i < src.m_nElementCount; i++) m_pData[nAppenedeAt + i] = src.m_pData[i];
...
BOOL CArrayEx<TYPE, ARG_TYPE>::_MakeSpaces(int nIndex, int nCount /*= 1*/) {
  ...
  for (int i = m_nElementCount - 1; i >= nIndex; i--) m_pData[i + nCount] = m_pData[i];
...
BOOL CArrayEx<TYPE, ARG_TYPE>::_GrowSize(int nAdd) {
  ...
  while (m_nAllocSize < m_nElementCount + nAdd) m_nAllocSize *= 2;
  ...
  if (m_pData != NULL) {
    for (int i = 0; i < m_nElementCount; i++) pNew[i] = m_pData[i];
    delete[] m_pData;
  }
  m_pData = pNew;
  return TRUE;
