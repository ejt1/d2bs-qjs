...
static uint32_t RoundUpToNextValidAsmJSHeapLength(uint32_t length) {
  if (length <= MinHeapLength) return MinHeapLength;
...
struct js::AsmJSMetadata : Metadata, AsmJSMetadataCacheablePod {
  ...
  const AsmJSExport& lookupAsmJSExport(uint32_t funcIndex) const {
    ...
    for (const AsmJSExport& exp : asmJSExports) {
      if (exp.funcIndex() == funcIndex) return exp;
    }
  ...
  bool getFuncName(const Bytes* maybeBytecode, uint32_t funcIndex,
...
static inline ParseNode* MaybeInitializer(ParseNode* pn) { return pn->expr(); }
...
static inline ParseNode* SkipEmptyStatements(ParseNode* pn) {
  while (pn && IsEmptyStatement(pn)) pn = pn->pn_next;
  return pn;
}
...
static bool GetToken(AsmJSParser& parser, TokenKind* tkp) {
  ...
  TokenKind tk;
  while (true) {
    if (!ts.getToken(&tk, TokenStreamShared::Operand)) return false;
...
static bool PeekToken(AsmJSParser& parser, TokenKind* tkp) {
  ...
  TokenKind tk;
  while (true) {
    if (!ts.peekToken(&tk, TokenStream::Operand)) return false;
    if (tk != TokenKind::Semi) break;
...
static bool ParseVarOrConstStatement(AsmJSParser& parser, ParseNode** var) {
  TokenKind tk;
  if (!PeekToken(parser, &tk)) return false;
...
class Type {
 ...
 public:
  ...
  bool isInt8x16() const { return which_ == Int8x16; }

  bool isInt16x8() const { return which_ == Int16x8; }

  bool isInt32x4() const { return which_ == Int32x4; }

  bool isUint8x16() const { return which_ == Uint8x16; }

  bool isUint16x8() const { return which_ == Uint16x8; }

  bool isUint32x4() const { return which_ == Uint32x4; }

  bool isFloat32x4() const { return which_ == Float32x4; }

  bool isBool8x16() const { return which_ == Bool8x16; }

  bool isBool16x8() const { return which_ == Bool16x8; }

  bool isBool32x4() const { return which_ == Bool32x4; }

  bool isSimd() const {
    return isInt8x16() || isInt16x8() || isInt32x4() || isUint8x16() ||
           isUint16x8() || isUint32x4() || isFloat32x4() || isBool8x16() ||
           isBool16x8() || isBool32x4();
  }
  ...
  bool isArgType() const {
    return isInt() || isFloat() || isDouble() ||
           (isSimd() && !isUnsignedSimd());
  }

  // Check if this is one of the valid types for a function return value.
  bool isReturnType() const {
    return isSigned() || isFloat() || isDouble() ||
           (isSimd() && !isUnsignedSimd()) || isVoid();
  }
  ...
  ExprType canonicalToExprType() const {
    switch (which()) {
      ...
      case Uint8x16:
      case Int8x16:
        return ExprType::I8x16;
      case Uint16x8:
      case Int16x8:
        return ExprType::I16x8;
      case Uint32x4:
      case Int32x4:
        return ExprType::I32x4;
      case Float32x4:
        return ExprType::F32x4;
      case Bool8x16:
        return ExprType::B8x16;
      case Bool16x8:
        return ExprType::B16x8;
      case Bool32x4:
        return ExprType::B32x4;
      default:
...
class MOZ_STACK_CLASS ModuleValidator {
 public:
  ...
  class Global {
   ...
   public:
    ...
    bool isAtomicsFunction() const { return which_ == AtomicsBuiltinFunction; }
    AsmJSAtomicsBuiltinFunction atomicsBuiltinFunction() const {
      MOZ_ASSERT(which_ == AtomicsBuiltinFunction);
      return u.atomicsBuiltinFunc_;
    }
    bool isSimdCtor() const { return which_ == SimdCtor; }
    SimdType simdCtorType() const {
      MOZ_ASSERT(which_ == SimdCtor);
      return u.simdCtorType_;
    }
    bool isSimdOperation() const { return which_ == SimdOp; }
    SimdOperation simdOperation() const {
 ...
 private:
  class HashableSig {
   ...
   public:
    ...
    uint32_t sigIndex() const { return sigIndex_; }
    const Sig& sig() const { return sigs_[sigIndex_]; }

    // Implement HashPolicy:
    typedef const Sig& Lookup;
    static HashNumber hash(Lookup l) { return l.hash(); }
    static bool match(HashableSig lhs, Lookup rhs) { return lhs.sig() == rhs; }
  };
  ...
  bool addStandardLibraryAtomicsName(const char* name,
 ...
 public:
  ...
  ~ModuleValidator() {
    ...
    if (errorOverRecursed_) ReportOverRecursed(cx_);
  }
    ...
    if (ts.computeErrorMetadata(&metadata, offset)) {
      if (ts.anyCharsAccess().options().throwOnAsmJSValidationFailureOption) {
        ...
        Unused << ts.anyCharsAccess().compileWarning(
            Move(metadata), nullptr, JSREPORT_WARNING, JSMSG_USE_ASM_TYPE_FAIL,
            args);
      }
    }
 ...
 public:
  bool init() {
    ...
    if (!asmJSMetadata_) return false;

    asmJSMetadata_->toStringStart =
        moduleFunctionNode_->pn_funbox->toStringStart;
    ...
    if (!standardLibraryAtomicsNames_.init() ||
        !addStandardLibraryAtomicsName("compareExchange",
                                       AsmJSAtomicsBuiltin_compareExchange) ||
        !addStandardLibraryAtomicsName("exchange",
                                       AsmJSAtomicsBuiltin_exchange) ||
        ...
        !addStandardLibraryAtomicsName("isLockFree",
                                       AsmJSAtomicsBuiltin_isLockFree)) {
      return false;
    }

#define ADDSTDLIBSIMDOPNAME(op) \
  || !addStandardLibrarySimdOpName(#op, SimdOperation::Fn_##op)
...
#undef ADDSTDLIBSIMDOPNAME
    ...
    if (!dummyFunction_) return false;
  ...
  MOZ_MUST_USE bool initGlobalArgumentName(PropertyName* n) {
    ...
    if (n) {
      ...
      if (!asmJSMetadata_->globalArgumentName) return false;
    }
    return true;
  }
  MOZ_MUST_USE bool initImportArgumentName(PropertyName* n) {
    ...
    if (n) {
      ...
      if (!asmJSMetadata_->importArgumentName) return false;
    }
    return true;
  }
  MOZ_MUST_USE bool initBufferArgumentName(PropertyName* n) {
    ...
    if (n) {
      ...
      if (!asmJSMetadata_->bufferArgumentName) return false;
    }
    return true;
  }
  bool addGlobalVarInit(PropertyName* var, const NumLit& lit, Type type,
    ...
    if (isConst) global->u.varOrConst.literalValue_ = lit;
    if (!globalMap_.putNew(var, global)) return false;
  ...
  bool addGlobalVarImport(PropertyName* var, PropertyName* field, Type type,
    ...
    if (!fieldChars) return false;

    uint32_t index = env_.globals.length();
    ValType valType = type.canonicalToValType();
    if (!env_.globals.emplaceBack(valType, !isConst, index)) return false;
  ...
  bool addArrayView(PropertyName* var, Scalar::Type vt,
    ...
    if (!arrayViews_.append(ArrayView(var, vt))) return false;
 ...
 private:
  bool addGlobalDoubleConstant(PropertyName* var, double constant) {
    ...
    global->u.varOrConst.literalValue_ =
        NumLit(NumLit::Double, DoubleValue(constant));
 ...
 public:
  bool addMathBuiltinConstant(PropertyName* var, double constant,
    ...
    if (!addGlobalDoubleConstant(var, constant)) return false;
  ...
  bool addAtomicsBuiltinFunction(PropertyName* var,
                                 AsmJSAtomicsBuiltinFunction func,
                                 PropertyName* field) {
    ...
    Global* global =
        validationLifo_.new_<Global>(Global::AtomicsBuiltinFunction);
    if (!global) return false;
  ...
  bool addSimdOperation(PropertyName* var, SimdType type, SimdOperation op,
                        PropertyName* field) {
    simdPresent_ = true;
  ...
  bool addFFI(PropertyName* var, PropertyName* field) {
    ...
    if (asmJSMetadata_->numFFIs == UINT32_MAX) return false;
  ...
  bool addExportField(const Func& func, PropertyName* maybeField) {
    ...
    if (!env_.exports.emplaceBack(Move(fieldChars), funcIndex,
  ...
  bool addFuncDef(PropertyName* name, uint32_t firstUse, Sig&& sig,
                  Func** func) {
    uint32_t sigIndex;
    if (!declareSig(Move(sig), &sigIndex)) return false;
    ...
    if (!globalMap_.putNew(name, global)) return false;
    if (!funcDefs_.emplaceBack(name, sigIndex, firstUse, funcDefIndex))
      return false;
    *func = &funcDefs_.back();
    return true;
  }
  bool declareFuncPtrTable(Sig&& sig, PropertyName* name, uint32_t firstUse,
                           uint32_t mask, uint32_t* tableIndex) {
    ...
    *tableIndex = env_.tables.length();
    ...
    uint32_t sigIndex;
    if (!newSig(Move(sig), &sigIndex)) return false;

    MOZ_ASSERT(sigIndex >= env_.asmJSSigToTableIndex.length());
    if (!env_.asmJSSigToTableIndex.resize(sigIndex + 1)) return false;
  ...
  bool defineFuncPtrTable(uint32_t tableIndex, Uint32Vector&& elems) {
    ...
    if (table.defined()) return false;

    table.define();

    for (uint32_t& index : elems) index += funcImportMap_.count();

    return env_.elemSegments.emplaceBack(tableIndex, InitExpr(Val(uint32_t(0))),
                                         Move(elems));
  }
  bool declareImport(PropertyName* name, Sig&& sig, unsigned ffiIndex,
    ...
    *importIndex = funcImportMap_.count();
    ...
    if (!asmJSMetadata_->asmJSImports.emplaceBack(ffiIndex)) return false;

    uint32_t sigIndex;
    if (!declareSig(Move(sig), &sigIndex)) return false;

    return funcImportMap_.add(p, NamedSig(name, sigIndex, env_.sigs),
                              *importIndex);
  }
  ...
  bool tryConstantAccess(uint64_t start, uint64_t width) {
    ...
    if (len > uint64_t(INT32_MAX) + 1) return false;
    len = RoundUpToNextValidAsmJSHeapLength(len);
    if (len > env_.minMemoryLength) env_.minMemoryLength = len;
    return true;
  }
  ...
  const Global* lookupGlobal(PropertyName* name) const {
    if (GlobalMap::Ptr p = globalMap_.lookup(name)) return p->value();
    return nullptr;
  }
  ...
  bool lookupStandardLibraryAtomicsName(
      PropertyName* name, AsmJSAtomicsBuiltinFunction* atomicsBuiltin) const {
  ...
  bool lookupStandardSimdOpName(PropertyName* name, SimdOperation* op) const {
    if (SimdOperationNameMap::Ptr p =
            standardLibrarySimdOpNames_.lookup(name)) {
      *op = p->value();
      return true;
    }
    return false;
  ...
  bool startFunctionBodies() {
    if (!arrayViews_.empty())
      env_.memoryUsage =
          atomicsPresent_ ? MemoryUsage::Shared : MemoryUsage::Unshared;
    else
  ...
  SharedModule finish() {
    ...
    for (const Func& func : funcDefs_) {
      ...
      if (!funcName ||
          !asmJSMetadata_->asmJSFuncNames.emplaceBack(Move(funcName)))
        return nullptr;
    }
    ...
    if (parser_.ss->filename()) {
      ...
      if (!scriptedCaller.filename) return nullptr;
    }
    ...
    for (const Func& func : funcDefs_) codeSectionSize += func.bytes().length();
    ...
    if (!mg.init(asmJSMetadata_.get())) return nullptr;
    ...
    if (!mg.finishFuncDefs()) return nullptr;
...
static bool IsCallToGlobal(ModuleValidator& m, ParseNode* pn,
  ...
  if (!pn->isKind(ParseNodeKind::Call)) return false;

  ParseNode* callee = CallCallee(pn);
  if (!callee->isKind(ParseNodeKind::Name)) return false;
...
static bool IsCoercionCall(ModuleValidator& m, ParseNode* pn, Type* coerceTo,
  ...
  const ModuleValidator::Global* global;
  if (!IsCallToGlobal(m, pn, &global)) return false;

  if (CallArgListLength(pn) != 1) return false;

  if (coercedExpr) *coercedExpr = CallArgList(pn);
  ...
  if (global->isSimdOperation() &&
      global->simdOperation() == SimdOperation::Fn_check) {
...
static bool IsFloatLiteral(ModuleValidator& m, ParseNode* pn) {
  ParseNode* coercedExpr;
  Type coerceTo;
  if (!IsCoercionCall(m, pn, &coerceTo, &coercedExpr)) return false;
  // Don't fold into || to avoid clang/memcheck bug (bug 1077031).
  if (!coerceTo.isFloat()) return false;
  return IsNumericNonFloatLiteral(coercedExpr);
}

static bool IsSimdTuple(ModuleValidator& m, ParseNode* pn, SimdType* type) {
  const ModuleValidator::Global* global;
  if (!IsCallToGlobal(m, pn, &global)) return false;

  if (!global->isSimdCtor()) return false;
...
static bool IsSimdLiteral(ModuleValidator& m, ParseNode* pn) {
  SimdType type;
  if (!IsSimdTuple(m, pn, &type)) return false;

  ParseNode* arg = CallArgList(pn);
  unsigned length = GetSimdLanes(type);
  for (unsigned i = 0; i < length; i++) {
    if (!IsNumericLiteral(m, arg)) return false;
    ...
    switch (type) {
      ...
      case SimdType::Bool16x8:
      case SimdType::Bool32x4:
        if (!IsLiteralInt(m, arg, &_)) return false;
        break;
      case SimdType::Float32x4:
        if (!IsNumericNonFloatLiteral(arg)) return false;
        break;
      default:
...
static NumLit ExtractSimdValue(ModuleValidator& m, ParseNode* pn) {
  ...
  switch (type) {
    ...
    case SimdType::Uint8x16: {
      ...
      MOZ_ASSERT(arg == nullptr);
      NumLit::Which w =
          type == SimdType::Uint8x16 ? NumLit::Uint8x16 : NumLit::Int8x16;
    ...
    case SimdType::Uint16x8: {
      ...
      MOZ_ASSERT(arg == nullptr);
      NumLit::Which w =
          type == SimdType::Uint16x8 ? NumLit::Uint16x8 : NumLit::Int16x8;
    ...
    case SimdType::Uint32x4: {
      ...
      MOZ_ASSERT(arg == nullptr);
      NumLit::Which w =
          type == SimdType::Uint32x4 ? NumLit::Uint32x4 : NumLit::Int32x4;
...
static NumLit ExtractNumericLiteral(ModuleValidator& m, ParseNode* pn) {
  ...
  if (i64 >= 0) {
    if (i64 <= INT32_MAX) return NumLit(NumLit::Fixnum, Int32Value(i64));
...
#define ENUMERATE(TYPE, FOR_ALL, DO)   \
  switch (op) {                        \
    case SimdOperation::Constructor:   \
      return MozOp::TYPE##Constructor; \
      FOR_ALL(DO)                      \
    default:                           \
      break;                           \
  }

static inline MozOp SimdToOp(SimdType type, SimdOperation op) {
  switch (type) {
    case SimdType::Uint8x16:
      // Handle the special unsigned opcodes, then fall through to Int8x16.
      switch (op) {
        case SimdOperation::Fn_addSaturate:
          return MozOp::I8x16addSaturateU;
        case SimdOperation::Fn_subSaturate:
          return MozOp::I8x16subSaturateU;
        case SimdOperation::Fn_extractLane:
          return MozOp::I8x16extractLaneU;
        case SimdOperation::Fn_shiftRightByScalar:
          return MozOp::I8x16shiftRightByScalarU;
        case SimdOperation::Fn_lessThan:
          return MozOp::I8x16lessThanU;
        case SimdOperation::Fn_lessThanOrEqual:
          return MozOp::I8x16lessThanOrEqualU;
        case SimdOperation::Fn_greaterThan:
          return MozOp::I8x16greaterThanU;
        case SimdOperation::Fn_greaterThanOrEqual:
          return MozOp::I8x16greaterThanOrEqualU;
    ...
    case SimdType::Int8x16:
      ...
      switch (op) {
        ...
        case SimdOperation::Fn_fromUint16x8Bits:
          return MozOp::I8x16fromInt16x8Bits;
        case SimdOperation::Fn_fromUint32x4Bits:
          return MozOp::I8x16fromInt32x4Bits;
        default:
          break;
      }
    ...
    case SimdType::Uint16x8:
      ...
      switch (op) {
        case SimdOperation::Fn_addSaturate:
          return MozOp::I16x8addSaturateU;
        case SimdOperation::Fn_subSaturate:
          return MozOp::I16x8subSaturateU;
        case SimdOperation::Fn_extractLane:
          return MozOp::I16x8extractLaneU;
        case SimdOperation::Fn_shiftRightByScalar:
          return MozOp::I16x8shiftRightByScalarU;
        case SimdOperation::Fn_lessThan:
          return MozOp::I16x8lessThanU;
        case SimdOperation::Fn_lessThanOrEqual:
          return MozOp::I16x8lessThanOrEqualU;
        case SimdOperation::Fn_greaterThan:
          return MozOp::I16x8greaterThanU;
        case SimdOperation::Fn_greaterThanOrEqual:
          return MozOp::I16x8greaterThanOrEqualU;
    ...
    case SimdType::Int16x8:
      ...
      switch (op) {
        case SimdOperation::Fn_fromUint8x16Bits:
          return MozOp::I16x8fromInt8x16Bits;
        case SimdOperation::Fn_fromUint16x8Bits:
          return MozOp::Limit;
        case SimdOperation::Fn_fromUint32x4Bits:
          return MozOp::I16x8fromInt32x4Bits;
        default:
          break;
      }
    ...
    case SimdType::Uint32x4:
      ...
      switch (op) {
        case SimdOperation::Fn_shiftRightByScalar:
          return MozOp::I32x4shiftRightByScalarU;
        case SimdOperation::Fn_lessThan:
          return MozOp::I32x4lessThanU;
        case SimdOperation::Fn_lessThanOrEqual:
          return MozOp::I32x4lessThanOrEqualU;
        case SimdOperation::Fn_greaterThan:
          return MozOp::I32x4greaterThanU;
        case SimdOperation::Fn_greaterThanOrEqual:
          return MozOp::I32x4greaterThanOrEqualU;
        case SimdOperation::Fn_fromFloat32x4:
          return MozOp::I32x4fromFloat32x4U;
    ...
    case SimdType::Int32x4:
      ...
      switch (op) {
        case SimdOperation::Fn_fromUint8x16Bits:
          return MozOp::I32x4fromInt8x16Bits;
        case SimdOperation::Fn_fromUint16x8Bits:
          return MozOp::I32x4fromInt16x8Bits;
        case SimdOperation::Fn_fromUint32x4Bits:
          return MozOp::Limit;
        default:
          break;
    ...
    case SimdType::Float32x4:
      switch (op) {
        case SimdOperation::Fn_fromUint8x16Bits:
          return MozOp::F32x4fromInt8x16Bits;
        case SimdOperation::Fn_fromUint16x8Bits:
          return MozOp::F32x4fromInt16x8Bits;
        case SimdOperation::Fn_fromUint32x4Bits:
          return MozOp::F32x4fromInt32x4Bits;
        default:
          break;
      }
...
class MOZ_STACK_CLASS FunctionValidator {
 ...
 public:
  ...
  bool init() {
    return locals_.init() && breakLabels_.init() && continueLabels_.init();
  }
  ...
  bool addLocal(ParseNode* pn, PropertyName* name, Type type) {
    ...
    if (p) return failName(pn, "duplicate local name '%s' not allowed", name);
 ...
 public:
  ...
  bool pushUnbreakableBlock(const NameVector* labels = nullptr) {
    if (labels) {
      for (PropertyName* label : *labels) {
        if (!breakLabels_.putNew(label, blockDepth_)) return false;
      }
    }
    blockDepth_++;
    return encoder().writeOp(Op::Block) &&
  ...
  bool popUnbreakableBlock(const NameVector* labels = nullptr) {
    if (labels) {
      for (PropertyName* label : *labels) removeLabel(label, &breakLabels_);
    }
    --blockDepth_;
    return encoder().writeOp(Op::End);
  }
  ...
  bool popIf(size_t typeAt, ExprType type) {
    MOZ_ASSERT(blockDepth_ > 0);
    --blockDepth_;
    if (!encoder().writeOp(Op::End)) return false;
  ...
  bool writeLabeledBreakOrContinue(PropertyName* label, bool isBreak) {
    ...
    if (LabelMap::Ptr p = map.lookup(label)) return writeBr(p->value());
  ...
  const Local* lookupLocal(PropertyName* name) const {
    if (auto p = locals_.lookup(name)) return &p->value();
    return nullptr;
  }

  const ModuleValidator::Global* lookupGlobal(PropertyName* name) const {
    if (locals_.has(name)) return nullptr;
  ...
  MOZ_MUST_USE bool writeCall(ParseNode* pn, Op op) {
    if (!encoder().writeOp(op)) return false;
...
static bool CheckModuleLevelName(ModuleValidator& m, ParseNode* usepn,
                                 PropertyName* name) {
  if (!CheckIdentifier(m, usepn, name)) return false;
...
static bool CheckFunctionHead(ModuleValidator& m, ParseNode* fn) {
  if (fn->pn_funbox->hasRest()) return m.fail(fn, "rest args not allowed");
...
static bool CheckArgument(ModuleValidator& m, ParseNode* arg,
  ...
  *name = nullptr;
  ...
  if (!CheckIdentifier(m, arg, arg->name())) return false;
...
static bool CheckModuleArgument(ModuleValidator& m, ParseNode* arg,
                                PropertyName** name) {
  if (!CheckArgument(m, arg, name)) return false;

  if (!CheckModuleLevelName(m, arg, *name)) return false;

  return true;
}

static bool CheckModuleArguments(ModuleValidator& m, ParseNode* fn) {
  unsigned numFormals;
  ...
  if (arg1 && !CheckModuleArgument(m, arg1, &arg1Name)) return false;
  if (!m.initGlobalArgumentName(arg1Name)) return false;

  PropertyName* arg2Name = nullptr;
  if (arg2 && !CheckModuleArgument(m, arg2, &arg2Name)) return false;
  if (!m.initImportArgumentName(arg2Name)) return false;

  PropertyName* arg3Name = nullptr;
  if (arg3 && !CheckModuleArgument(m, arg3, &arg3Name)) return false;
  if (!m.initBufferArgumentName(arg3Name)) return false;
...
static bool CheckTypeAnnotation(ModuleValidator& m, ParseNode* coercionNode,
  ...
  switch (coercionNode->getKind()) {
    case ParseNodeKind::BitOr: {
      ...
      *coerceTo = Type::Int;
      if (coercedExpr) *coercedExpr = BitwiseLeft(coercionNode);
      return true;
    }
    case ParseNodeKind::Pos: {
      *coerceTo = Type::Double;
      if (coercedExpr) *coercedExpr = UnaryKid(coercionNode);
      return true;
    }
    case ParseNodeKind::Call: {
      if (IsCoercionCall(m, coercionNode, coerceTo, coercedExpr)) return true;
      break;
    }
    default:;
  }

  return m.fail(coercionNode,
                "must be of the form +x, x|0, fround(x), or a SIMD check(x)");
}
...
static bool CheckGlobalVariableInitImport(ModuleValidator& m,
  ...
  Type coerceTo;
  ParseNode* coercedExpr;
  if (!CheckTypeAnnotation(m, initNode, &coerceTo, &coercedExpr)) return false;
...
static bool CheckNewArrayViewArgs(ModuleValidator& m, ParseNode* ctorExpr,
...
static bool CheckNewArrayView(ModuleValidator& m, PropertyName* varName,
  ...
  if (!CheckNewArrayViewArgs(m, ctorExpr, bufferName)) return false;

  return m.addArrayView(varName, type, field);
}

static bool IsSimdValidOperationType(SimdType type, SimdOperation op) {
...
static bool CheckGlobalAtomicsImport(ModuleValidator& m, ParseNode* initNode,
                                     PropertyName* varName,
                                     PropertyName* field) {
  // Atomics builtin, with the form glob.Atomics.[[builtin]]
  AsmJSAtomicsBuiltinFunction func;
  if (!m.lookupStandardLibraryAtomicsName(field, &func))
    return m.failName(initNode, "'%s' is not a standard Atomics builtin",
                      field);

  return m.addAtomicsBuiltinFunction(varName, func, field);
}

static bool CheckGlobalSimdImport(ModuleValidator& m, ParseNode* initNode,
                                  PropertyName* varName, PropertyName* field) {
  if (!m.supportsSimd())
...
static bool CheckGlobalSimdOperationImport(
    ModuleValidator& m, const ModuleValidator::Global* global,
  ...
  if (!m.lookupStandardSimdOpName(opName, &simdOp))
    return m.failName(initNode, "'%s' is not a standard SIMD operation",
                      opName);
  if (!IsSimdValidOperationType(simdType, simdOp))
    return m.failName(initNode,
                      "'%s' is not an operation supported by the SIMD type",
                      opName);
...
static bool CheckGlobalDotImport(ModuleValidator& m, PropertyName* varName,
  ...
  if (base->name() == m.importArgumentName()) return m.addFFI(varName, field);

  const ModuleValidator::Global* global = m.lookupGlobal(base->name());
  if (!global)
    return m.failName(initNode, "%s not found in module global scope",
                      base->name());
...
static bool CheckModuleGlobal(ModuleValidator& m, ParseNode* var,
  ...
  if (!CheckModuleLevelName(m, var, var->name())) return false;

  ParseNode* initNode = MaybeInitializer(var);
  if (!initNode) return m.fail(var, "module import needs initializer");
...
static bool CheckModuleProcessingDirectives(ModuleValidator& m) {
  ...
  while (true) {
    ...
    TokenKind tt;
    if (!ts.getToken(&tt)) return false;
...
static bool CheckModuleGlobals(ModuleValidator& m) {
  while (true) {
    ParseNode* varStmt;
    if (!ParseVarOrConstStatement(m.parser(), &varStmt)) return false;
    if (!varStmt) break;
...
static bool CheckArgumentType(FunctionValidator& f, ParseNode* stmt,
  ...
  if (!initNode->isKind(ParseNodeKind::Assign)) return ArgFail(f, name, stmt);

  ParseNode* argNode = BinaryLeft(initNode);
  ParseNode* coercionNode = BinaryRight(initNode);

  if (!IsUseOfName(argNode, name)) return ArgFail(f, name, stmt);
  ...
  if (!IsUseOfName(coercedExpr, name)) return ArgFail(f, name, stmt);
...
static bool CheckProcessingDirectives(ModuleValidator& m,
  ...
  while (stmt && IsIgnoredDirective(m.cx(), stmt)) stmt = NextNode(stmt);
...
static bool CheckArguments(FunctionValidator& f, ParseNode** stmtIter,
  ...
  for (unsigned i = 0; i < numFormals;
    ...
    PropertyName* name;
    if (!CheckArgument(f.m(), argpn, &name)) return false;

    Type type;
    if (!CheckArgumentType(f, stmt, name, &type)) return false;

    if (!argTypes->append(type.canonicalToValType())) return false;

    if (!f.addLocal(argpn, name, type)) return false;
  }
...
static bool IsLiteralOrConst(FunctionValidator& f, ParseNode* pn, NumLit* lit) {
  ...
  if (!IsNumericLiteral(f.m(), pn)) return false;
...
static bool CheckFinalReturn(FunctionValidator& f,
  ...
  if (!f.encoder().writeOp(Op::End)) return false;
...
static bool CheckVariable(FunctionValidator& f, ParseNode* var,
  ...
  if (!CheckIdentifier(f.m(), var, name)) return false;
...
static bool CheckVariables(FunctionValidator& f, ParseNode** stmtIter) {
  ...
  for (; stmt && stmt->isKind(ParseNodeKind::Var);
    ...
    for (ParseNode* var = VarListHead(stmt); var; var = NextNode(var)) {
      if (!CheckVariable(f, var, &types, &inits)) return false;
    }
  }

  MOZ_ASSERT(f.encoder().empty());

  if (!EncodeLocalEntries(f.encoder(), types)) return false;

  for (uint32_t i = 0; i < inits.length(); i++) {
    NumLit lit = inits[i];
    if (lit.isZeroBits()) continue;
    if (!f.writeConstExpr(lit)) return false;
    if (!f.encoder().writeOp(Op::SetLocal)) return false;
    if (!f.encoder().writeVarU32(firstVar + i)) return false;
  }
...
static bool CheckVarRef(FunctionValidator& f, ParseNode* varRef, Type* type) {
  ...
  if (const FunctionValidator::Local* local = f.lookupLocal(name)) {
    if (!f.encoder().writeOp(Op::GetLocal)) return false;
    if (!f.encoder().writeVarU32(local->slot)) return false;
    *type = local->type;
    return true;
  }
...
static inline bool IsLiteralOrConstInt(FunctionValidator& f, ParseNode* pn,
                                       uint32_t* u32) {
  NumLit lit;
  if (!IsLiteralOrConst(f, pn, &lit)) return false;
...
static bool CheckArrayAccess(FunctionValidator& f, ParseNode* viewName,
                             ParseNode* indexExpr, bool isSimd,
  ...
  *viewType = global->viewType();
  ...
  if (indexExpr->isKind(ParseNodeKind::Rsh)) {
    ...
    Type pointerType;
    if (!CheckExpr(f, pointerNode, &pointerType)) return false;
    ...
    if (isSimd) {
      if (!pointerType.isIntish())
        return f.failf(pointerNode, "%s is not a subtype of intish",
                       pointerType.toChars());
    } else {
  ...
  // for a shift of zero or a SIMD access.
  if (mask != NoMask) {
...
static bool CheckAndPrepareArrayAccess(FunctionValidator& f,
                                       ParseNode* viewName,
                                       ParseNode* indexExpr, bool isSimd,
                                       Scalar::Type* viewType) {
...
static bool WriteArrayAccessFlags(FunctionValidator& f, Scalar::Type viewType) {
  ...
  if (!f.encoder().writeFixedU8(CeilingLog2(align))) return false;

  // asm.js doesn't have constant offsets, so just encode a 0.
  if (!f.encoder().writeVarU32(0)) return false;
...
static bool CheckLoadArray(FunctionValidator& f, ParseNode* elem, Type* type) {
  ...
  if (!CheckAndPrepareArrayAccess(f, ElemBase(elem), ElemIndex(elem), NoSimd,
                                  &viewType))
    return false;
  ...
  if (!WriteArrayAccessFlags(f, viewType)) return false;
...
static bool CheckStoreArray(FunctionValidator& f, ParseNode* lhs,
  ...
  Scalar::Type viewType;
  if (!CheckAndPrepareArrayAccess(f, ElemBase(lhs), ElemIndex(lhs), NoSimd,
                                  &viewType))
    return false;

  Type rhsType;
  if (!CheckExpr(f, rhs, &rhsType)) return false;
  ...
  switch (viewType) {
    case Scalar::Int8:
    case Scalar::Uint8:
      if (!f.encoder().writeOp(MozOp::I32TeeStore8)) return false;
      break;
    case Scalar::Int16:
    case Scalar::Uint16:
      if (!f.encoder().writeOp(MozOp::I32TeeStore16)) return false;
      break;
    case Scalar::Int32:
    case Scalar::Uint32:
      if (!f.encoder().writeOp(MozOp::I32TeeStore)) return false;
      break;
    case Scalar::Float32:
      if (rhsType.isFloatish()) {
        if (!f.encoder().writeOp(MozOp::F32TeeStore)) return false;
      } else {
        if (!f.encoder().writeOp(MozOp::F64TeeStoreF32)) return false;
      }
      break;
    case Scalar::Float64:
      if (rhsType.isFloatish()) {
        if (!f.encoder().writeOp(MozOp::F32TeeStoreF64)) return false;
      } else {
        if (!f.encoder().writeOp(MozOp::F64TeeStore)) return false;
      }
      break;
    default:
      MOZ_CRASH("unexpected scalar type");
...
static bool CheckAssignName(FunctionValidator& f, ParseNode* lhs,
  ...
  if (const FunctionValidator::Local* lhsVar = f.lookupLocal(name)) {
    ...
    if (!f.encoder().writeOp(Op::TeeLocal)) return false;
    if (!f.encoder().writeVarU32(lhsVar->slot)) return false;
  ...
  if (const ModuleValidator::Global* global = f.lookupGlobal(name)) {
    ...
    if (!(rhsType <= globType))
      return f.failf(lhs, "%s is not a subtype of %s", rhsType.toChars(),
                     globType.toChars());
    if (!f.encoder().writeOp(MozOp::TeeGlobal)) return false;
    if (!f.encoder().writeVarU32(global->varOrConstIndex())) return false;
...
static bool CheckMathIMul(FunctionValidator& f, ParseNode* call, Type* type) {
  ...
  Type lhsType;
  if (!CheckExpr(f, lhs, &lhsType)) return false;
...
static bool CheckMathClz32(FunctionValidator& f, ParseNode* call, Type* type) {
  ...
  Type argType;
  if (!CheckExpr(f, arg, &argType)) return false;
...
static bool CheckMathMinMax(FunctionValidator& f, ParseNode* callNode,
  ...
  Type firstType;
  if (!CheckExpr(f, firstArg, &firstType)) return false;
  ...
  for (unsigned i = 1; i < numArgs; i++, nextArg = NextNode(nextArg)) {
    Type nextType;
    if (!CheckExpr(f, nextArg, &nextType)) return false;
    if (!(nextType <= firstType))
      return f.failf(nextArg, "%s is not a subtype of %s", nextType.toChars(),
                     firstType.toChars());

    if (op != Op::Limit) {
      if (!f.encoder().writeOp(op)) return false;
    } else {
      if (!f.encoder().writeOp(mozOp)) return false;
    }
  }
...
static bool CheckSharedArrayAtomicAccess(FunctionValidator& f,
                                         ParseNode* viewName,
                                         ParseNode* indexExpr,
                                         Scalar::Type* viewType) {
...
static bool WriteAtomicOperator(FunctionValidator& f, MozOp opcode,
                                Scalar::Type viewType) {
  return f.encoder().writeOp(opcode) && f.encoder().writeFixedU8(viewType);
}

static bool CheckAtomicsLoad(FunctionValidator& f, ParseNode* call,
                             Type* type) {
  if (CallArgListLength(call) != 2)
  ...
  if (!WriteAtomicOperator(f, MozOp::I32AtomicsLoad, viewType)) return false;
...
static bool CheckAtomicsStore(FunctionValidator& f, ParseNode* call,
                              Type* type) {
  ...
  Type rhsType;
  if (!CheckExpr(f, valueArg, &rhsType)) return false;

  if (!rhsType.isIntish())
    return f.failf(arrayArg, "%s is not a subtype of intish",
                   rhsType.toChars());
  ...
  if (!WriteAtomicOperator(f, MozOp::I32AtomicsStore, viewType)) return false;
...
static bool CheckAtomicsBinop(FunctionValidator& f, ParseNode* call, Type* type,
                              AtomicOp op) {
  if (CallArgListLength(call) != 3)
  ...
  Type valueArgType;
  if (!CheckExpr(f, valueArg, &valueArgType)) return false;

  if (!valueArgType.isIntish())
    return f.failf(valueArg, "%s is not a subtype of intish",
                   valueArgType.toChars());

  Scalar::Type viewType;
  if (!CheckSharedArrayAtomicAccess(f, arrayArg, indexArg, &viewType))
    return false;

  if (!WriteAtomicOperator(f, MozOp::I32AtomicsBinOp, viewType)) return false;
  if (!f.encoder().writeFixedU8(uint8_t(op))) return false;
...
static bool CheckAtomicsIsLockFree(FunctionValidator& f, ParseNode* call,
                                   Type* type) {
  if (CallArgListLength(call) != 1)
  ...
  if (!IsLiteralInt(f.m(), sizeArg, &size))
    return f.fail(sizeArg,
                  "Atomics.isLockFree requires an integer literal argument");

  *type = Type::Int;
  return f.writeInt32Lit(AtomicOperations::isLockfreeJS(size));
}

static bool CheckAtomicsCompareExchange(FunctionValidator& f, ParseNode* call,
                                        Type* type) {
  ...
  if (!CheckExpr(f, oldValueArg, &oldValueArgType)) return false;

  Type newValueArgType;
  if (!CheckExpr(f, newValueArg, &newValueArgType)) return false;

  if (!oldValueArgType.isIntish())
    return f.failf(oldValueArg, "%s is not a subtype of intish",
                   oldValueArgType.toChars());

  if (!newValueArgType.isIntish())
    return f.failf(newValueArg, "%s is not a subtype of intish",
                   newValueArgType.toChars());
...
static bool CheckAtomicsExchange(FunctionValidator& f, ParseNode* call,
                                 Type* type) {
  if (CallArgListLength(call) != 3)
...
static bool CheckAtomicsBuiltinCall(FunctionValidator& f, ParseNode* callNode,
                                    AsmJSAtomicsBuiltinFunction func,
                                    Type* type) {
  switch (func) {
...
static bool CheckCallArgs(FunctionValidator& f, ParseNode* callNode,
  ...
  for (unsigned i = 0; i < CallArgListLength(callNode);
    ...
    Type type;
    if (!CheckExpr(f, argNode, &type)) return false;

    if (!checkArg(f, argNode, type)) return false;
...
static bool CheckSignatureAgainstExisting(ModuleValidator& m, ParseNode* usepn,
                                          const Sig& sig, const Sig& existing) {
...
static bool CheckFunctionSignature(ModuleValidator& m, ParseNode* usepn,
  ...
  if (!existing) {
    if (!CheckModuleLevelName(m, usepn, name)) return false;
    return m.addFuncDef(name, usepn->pn_pos.begin, Move(sig), func);
  }

  const SigWithId& existingSig = m.env().sigs[existing->sigIndex()];

  if (!CheckSignatureAgainstExisting(m, usepn, sig, existingSig)) return false;
...
static bool CheckInternalCall(FunctionValidator& f, ParseNode* callNode,
  ...
  ValTypeVector args;
  if (!CheckCallArgs<CheckIsArgType>(f, callNode, &args)) return false;
  ...
  if (!f.writeCall(callNode, MozOp::OldCallDirect)) return false;

  if (!f.encoder().writeVarU32(callee->funcDefIndex())) return false;
...
static bool CheckFuncPtrTableAgainstExisting(ModuleValidator& m,
  ...
  if (const ModuleValidator::Global* existing = m.lookupGlobal(name)) {
    ...
    if (!CheckSignatureAgainstExisting(m, usepn, sig,
                                       m.env().sigs[table.sigIndex()]))
      return false;
  ...
  if (!m.declareFuncPtrTable(Move(sig), name, usepn->pn_pos.begin, mask,
                             tableIndex))
    return false;
...
static bool CheckFuncPtrCall(FunctionValidator& f, ParseNode* callNode,
  ...
  Type indexType;
  if (!CheckExpr(f, indexNode, &indexType)) return false;
  ...
  uint32_t tableIndex;
  if (!CheckFuncPtrTableAgainstExisting(f.m(), tableNode, name, Move(sig), mask,
                                        &tableIndex))
    return false;

  if (!f.writeCall(callNode, MozOp::OldCallIndirect)) return false;
...
static bool CheckFFICall(FunctionValidator& f, ParseNode* callNode,
  ...
  if (ret.isFloat()) return f.fail(callNode, "FFI calls can't return float");
  if (ret.isSimd())
    return f.fail(callNode, "FFI calls can't return SIMD values");

  ValTypeVector args;
  if (!CheckCallArgs<CheckIsExternType>(f, callNode, &args)) return false;
  ...
  if (!f.writeCall(callNode, Op::Call)) return false;

  if (!f.encoder().writeVarU32(importIndex)) return false;
...
static bool CheckFloatCoercionArg(FunctionValidator& f, ParseNode* inputNode,
                                  Type inputType) {
  if (inputType.isMaybeDouble()) return f.encoder().writeOp(Op::F32DemoteF64);
  if (inputType.isSigned()) return f.encoder().writeOp(Op::F32ConvertSI32);
  if (inputType.isUnsigned()) return f.encoder().writeOp(Op::F32ConvertUI32);
  if (inputType.isFloatish()) return true;
...
static bool CheckCoercionArg(FunctionValidator& f, ParseNode* arg,
  ...
  if (expected.isFloat()) {
    if (!CheckFloatCoercionArg(f, arg, argType)) return false;
  } else if (expected.isSimd()) {
    if (!(argType <= expected))
      return f.fail(
          arg, "argument to SIMD coercion isn't from the correct SIMD type");
  } else {
...
static bool CheckMathFRound(FunctionValidator& f, ParseNode* callNode,
  ...
  ParseNode* argNode = CallArgList(callNode);
  Type argType;
  if (!CheckCoercionArg(f, argNode, Type::Float, &argType)) return false;
...
static bool CheckMathBuiltinCall(FunctionValidator& f, ParseNode* callNode,
  ...
  if (!f.prepareCall(callNode)) return false;

  Type firstType;
  ParseNode* argNode = CallArgList(callNode);
  if (!CheckExpr(f, argNode, &firstType)) return false;
  ...
  if (arity == 2) {
    ...
    if (!CheckExpr(f, argNode, &secondType)) return false;
  ...
  if (opIsDouble) {
    if (f64 != Op::Limit) {
      if (!f.encoder().writeOp(f64)) return false;
    } else {
      if (!f.encoder().writeOp(mozf64)) return false;
    }
  } else {
    if (!f.encoder().writeOp(f32)) return false;
  }
...
static bool CheckSimdCallArgs(FunctionValidator& f, ParseNode* call,
                              unsigned expectedArity,
                              const CheckArgOp& checkArg) {
  unsigned numArgs = CallArgListLength(call);
  if (numArgs != expectedArity)
    return f.failf(call, "expected %u arguments to SIMD call, got %u",
                   expectedArity, numArgs);
  ...
  for (size_t i = 0; i < numArgs; i++, arg = NextNode(arg)) {
    ...
    Type argType;
    if (!CheckExpr(f, arg, &argType)) return false;
    if (!checkArg(f, arg, i, argType)) return false;
  }
...
class CheckArgIsSubtypeOf {
 ...
 public:
  ...
  bool operator()(FunctionValidator& f, ParseNode* arg, unsigned argIndex,
...
static inline Type SimdToCoercedScalarType(SimdType t) {
  switch (t) {
    case SimdType::Int8x16:
    case SimdType::Int16x8:
    case SimdType::Int32x4:
  ...
  bool operator()(FunctionValidator& f, ParseNode* arg, unsigned argIndex,
    ...
    if (!(actualType <= formalType_)) {
      ...
      if (simdType_ != SimdType::Float32x4 || !actualType.isDoubleLit()) {
        return f.failf(arg, "%s is not a subtype of %s%s", actualType.toChars(),
                       formalType_.toChars(),
...
class CheckSimdSelectArgs {
 ...
 public:
  explicit CheckSimdSelectArgs(SimdType t)
      : formalType_(t), maskType_(GetBooleanSimdType(t)) {}
...
static bool CheckSimdUnary(FunctionValidator& f, ParseNode* call,
                           SimdType opType, SimdOperation op, Type* type) {
  if (!CheckSimdCallArgs(f, call, 1, CheckArgIsSubtypeOf(opType))) return false;
  if (!f.writeSimdOp(opType, op)) return false;
  *type = opType;
  return true;
}

static bool CheckSimdBinaryShift(FunctionValidator& f, ParseNode* call,
                                 SimdType opType, SimdOperation op,
                                 Type* type) {
...
static bool CheckSimdBinaryComp(FunctionValidator& f, ParseNode* call,
                                SimdType opType, SimdOperation op, Type* type) {
  if (!CheckSimdCallArgs(f, call, 2, CheckArgIsSubtypeOf(opType))) return false;
  if (!f.writeSimdOp(opType, op)) return false;
  *type = GetBooleanSimdType(opType);
  return true;
}

static bool CheckSimdBinary(FunctionValidator& f, ParseNode* call,
                            SimdType opType, SimdOperation op, Type* type) {
  if (!CheckSimdCallArgs(f, call, 2, CheckArgIsSubtypeOf(opType))) return false;
  if (!f.writeSimdOp(opType, op)) return false;
  *type = opType;
...
static bool CheckSimdExtractLane(FunctionValidator& f, ParseNode* call,
                                 SimdType opType, Type* type) {
  switch (opType) {
    case SimdType::Int8x16:
    case SimdType::Int16x8:
    case SimdType::Int32x4:
  ...
  unsigned numArgs = CallArgListLength(call);
  if (numArgs != 2)
    return f.failf(call, "expected 2 arguments to SIMD extract, got %u",
                   numArgs);
  ...
  // First argument is the vector
  Type vecType;
  if (!CheckExpr(f, arg, &vecType)) return false;
  ...
  if (!f.writeSimdOp(opType, SimdOperation::Fn_extractLane)) return false;
  if (!f.encoder().writeVarU32(lane)) return false;
  return true;
}

static bool CheckSimdReplaceLane(FunctionValidator& f, ParseNode* call,
                                 SimdType opType, Type* type) {
  unsigned numArgs = CallArgListLength(call);
  if (numArgs != 3)
    return f.failf(call, "expected 2 arguments to SIMD replace, got %u",
                   numArgs);
  ...
  Type scalarType;
  if (!CheckExpr(f, arg, &scalarType)) return false;
  if (!(scalarType <= SimdToCoercedScalarType(opType))) {
    if (opType == SimdType::Float32x4 && scalarType.isDoubleLit()) {
      if (!f.encoder().writeOp(Op::F32DemoteF64)) return false;
    } else {
      return f.failf(arg,
                     "%s is not the correct type to replace an element of %s",
                     scalarType.toChars(), vecType.toChars());
    }
  }

  if (!f.writeSimdOp(opType, SimdOperation::Fn_replaceLane)) return false;
  if (!f.encoder().writeVarU32(lane)) return false;
  *type = opType;
  return true;
}
...
namespace {
// Include CheckSimdCast in unnamed namespace to avoid MSVC name lookup bug (due
// to the use of Type).

static bool CheckSimdCast(FunctionValidator& f, ParseNode* call,
                          SimdType fromType, SimdType toType, SimdOperation op,
                          Type* type) {
  if (!CheckSimdCallArgs(f, call, 1, CheckArgIsSubtypeOf(fromType)))
    return false;
  if (!f.writeSimdOp(toType, op)) return false;
  *type = toType;
  return true;
}
...
static bool CheckSimdShuffleSelectors(FunctionValidator& f, ParseNode* lane,
                                      mozilla::Array<uint8_t, 16>& lanes,
                                      unsigned numLanes, unsigned maxLane) {
  for (unsigned i = 0; i < numLanes; i++, lane = NextNode(lane)) {
    uint32_t u32;
    if (!IsLiteralInt(f.m(), lane, &u32))
      return f.failf(lane,
                     "lane selector should be a constant integer literal");
    if (u32 >= maxLane)
...
static bool CheckSimdSwizzle(FunctionValidator& f, ParseNode* call,
                             SimdType opType, Type* type) {
  const unsigned numLanes = GetSimdLanes(opType);
  unsigned numArgs = CallArgListLength(call);
  if (numArgs != 1 + numLanes)
    return f.failf(call, "expected %u arguments to SIMD swizzle, got %u",
                   1 + numLanes, numArgs);

  Type retType = opType;
  ParseNode* vec = CallArgList(call);
  Type vecType;
  if (!CheckExpr(f, vec, &vecType)) return false;
  if (!(vecType <= retType))
    return f.failf(vec, "%s is not a subtype of %s", vecType.toChars(),
                   retType.toChars());

  if (!f.writeSimdOp(opType, SimdOperation::Fn_swizzle)) return false;
  ...
  for (unsigned i = 0; i < numLanes; i++) {
    if (!f.encoder().writeFixedU8(lanes[i])) return false;
  }
...
static bool CheckSimdShuffle(FunctionValidator& f, ParseNode* call,
                             SimdType opType, Type* type) {
  const unsigned numLanes = GetSimdLanes(opType);
  unsigned numArgs = CallArgListLength(call);
  if (numArgs != 2 + numLanes)
    return f.failf(call, "expected %u arguments to SIMD shuffle, got %u",
                   2 + numLanes, numArgs);
  ...
  for (unsigned i = 0; i < 2; i++, arg = NextNode(arg)) {
    Type type;
    if (!CheckExpr(f, arg, &type)) return false;
    if (!(type <= retType))
      return f.failf(arg, "%s is not a subtype of %s", type.toChars(),
                     retType.toChars());
  }

  if (!f.writeSimdOp(opType, SimdOperation::Fn_shuffle)) return false;
  ...
  for (unsigned i = 0; i < numLanes; i++) {
    if (!f.encoder().writeFixedU8(uint8_t(lanes[i]))) return false;
  }
...
static bool CheckSimdLoadStoreArgs(FunctionValidator& f, ParseNode* call,
                                   Scalar::Type* viewType) {
  ParseNode* view = CallArgList(call);
  if (!view->isKind(ParseNodeKind::Name))
    return f.fail(
        view, "expected Uint8Array view as SIMD.*.load/store first argument");
...
static bool CheckSimdLoad(FunctionValidator& f, ParseNode* call,
                          SimdType opType, SimdOperation op, Type* type) {
  unsigned numArgs = CallArgListLength(call);
  if (numArgs != 2)
  ...
  Scalar::Type viewType;
  if (!CheckSimdLoadStoreArgs(f, call, &viewType)) return false;
...
static bool CheckSimdStore(FunctionValidator& f, ParseNode* call,
                           SimdType opType, SimdOperation op, Type* type) {
  unsigned numArgs = CallArgListLength(call);
  if (numArgs != 3)
  ...
  Type vecType;
  if (!CheckExpr(f, vecExpr, &vecType)) return false;
  ...
  if (!(vecType <= retType))
    return f.failf(vecExpr, "%s is not a subtype of %s", vecType.toChars(),
                   retType.toChars());
...
static bool CheckSimdSelect(FunctionValidator& f, ParseNode* call,
                            SimdType opType, Type* type) {
  if (!CheckSimdCallArgs(f, call, 3, CheckSimdSelectArgs(opType))) return false;
  if (!f.writeSimdOp(opType, SimdOperation::Fn_select)) return false;
  *type = opType;
  return true;
}

static bool CheckSimdAllTrue(FunctionValidator& f, ParseNode* call,
                             SimdType opType, Type* type) {
  if (!CheckSimdCallArgs(f, call, 1, CheckArgIsSubtypeOf(opType))) return false;
  if (!f.writeSimdOp(opType, SimdOperation::Fn_allTrue)) return false;
  *type = Type::Int;
  return true;
}

static bool CheckSimdAnyTrue(FunctionValidator& f, ParseNode* call,
                             SimdType opType, Type* type) {
  if (!CheckSimdCallArgs(f, call, 1, CheckArgIsSubtypeOf(opType))) return false;
  if (!f.writeSimdOp(opType, SimdOperation::Fn_anyTrue)) return false;
  *type = Type::Int;
  return true;
}

static bool CheckSimdCheck(FunctionValidator& f, ParseNode* call, Type* type) {
  Type coerceTo;
  ParseNode* argNode;
...
static bool CheckSimdSplat(FunctionValidator& f, ParseNode* call,
                           SimdType opType, Type* type) {
  if (!CheckSimdCallArgs(f, call, 1, CheckSimdScalarArgs(opType))) return false;
  if (!f.writeSimdOp(opType, SimdOperation::Fn_splat)) return false;
  *type = opType;
  return true;
}

static bool CheckSimdOperationCall(FunctionValidator& f, ParseNode* call,
                                   const ModuleValidator::Global* global,
                                   Type* type) {
...
#undef _CASE
    ...
    case SimdOperation::Fn_store3:
      return f.fail(call,
                    "asm.js does not support 3-element SIMD loads or stores");
...
static bool CheckSimdCtorCall(FunctionValidator& f, ParseNode* call,
                              const ModuleValidator::Global* global,
                              Type* type) {
  ...
  if (!f.writeSimdOp(simdType, SimdOperation::Constructor)) return false;
...
static bool CheckUncoercedCall(FunctionValidator& f, ParseNode* expr,
  ...
  if (IsCallToGlobal(f.m(), expr, &global)) {
    ...
    if (global->isAtomicsFunction())
      return CheckAtomicsBuiltinCall(f, expr, global->atomicsBuiltinFunction(),
                                     type);
    if (global->isSimdCtor()) return CheckSimdCtorCall(f, expr, global, type);
    if (global->isSimdOperation())
  ...
  return f.fail(
      expr,
      "all function calls must either be calls to standard lib math functions, "
...
static bool CoerceResult(FunctionValidator& f, ParseNode* expr, Type expected,
  ...
  switch (expected.which()) {
    case Type::Void:
      if (!actual.isVoid()) {
        if (!f.encoder().writeOp(Op::Drop)) return false;
      }
      break;
    case Type::Int:
      if (!actual.isIntish())
      ...
      break;
    case Type::Float:
      if (!CheckFloatCoercionArg(f, expr, actual)) return false;
      break;
    case Type::Double:
      if (actual.isMaybeDouble()) {
        // No conversion necessary.
      } else if (actual.isMaybeFloat()) {
        if (!f.encoder().writeOp(Op::F64PromoteF32)) return false;
      } else if (actual.isSigned()) {
        if (!f.encoder().writeOp(Op::F64ConvertSI32)) return false;
      } else if (actual.isUnsigned()) {
        if (!f.encoder().writeOp(Op::F64ConvertUI32)) return false;
      } else {
        return f.failf(
    ...
    default:
      ...
      if (actual != expected)
        return f.failf(expr, "got type %s, expected %s", actual.toChars(),
                       expected.toChars());
      break;
  }
...
static bool CheckCoercedMathBuiltinCall(FunctionValidator& f,
                                        ...
                                        Type* type) {
  Type actual;
  if (!CheckMathBuiltinCall(f, callNode, func, &actual)) return false;
  return CoerceResult(f, callNode, ret, actual, type);
}

static bool CheckCoercedSimdCall(FunctionValidator& f, ParseNode* call,
                                 const ModuleValidator::Global* global,
                                 Type ret, Type* type) {
  MOZ_ASSERT(ret.isCanonical());
  ...
  Type actual;
  if (global->isSimdCtor()) {
    if (!CheckSimdCtorCall(f, call, global, &actual)) return false;
    MOZ_ASSERT(actual.isSimd());
  } else {
    MOZ_ASSERT(global->isSimdOperation());
    if (!CheckSimdOperationCall(f, call, global, &actual)) return false;
  }

  return CoerceResult(f, call, ret, actual, type);
}

static bool CheckCoercedAtomicsBuiltinCall(FunctionValidator& f,
                                           ParseNode* callNode,
                                           AsmJSAtomicsBuiltinFunction func,
                                           Type ret, Type* type) {
  MOZ_ASSERT(ret.isCanonical());
  ...
  Type actual;
  if (!CheckAtomicsBuiltinCall(f, callNode, func, &actual)) return false;
  return CoerceResult(f, callNode, ret, actual, type);
}
...
static bool CheckCoercedCall(FunctionValidator& f, ParseNode* call, Type ret,
  ...
  if (!CheckRecursionLimitDontReport(f.cx())) return f.m().failOverRecursed();
  ...
  if (const ModuleValidator::Global* global = f.lookupGlobal(calleeName)) {
    switch (global->which()) {
      ...
      case ModuleValidator::Global::AtomicsBuiltinFunction:
        return CheckCoercedAtomicsBuiltinCall(
            f, call, global->atomicsBuiltinFunction(), ret, type);
      case ModuleValidator::Global::ConstantLiteral:
      case ModuleValidator::Global::ConstantImport:
      case ModuleValidator::Global::Variable:
      case ModuleValidator::Global::Table:
      case ModuleValidator::Global::ArrayView:
      case ModuleValidator::Global::ArrayViewCtor:
        return f.failName(callee, "'%s' is not callable function",
                          callee->name());
      case ModuleValidator::Global::SimdCtor:
      case ModuleValidator::Global::SimdOp:
...
static bool CheckPos(FunctionValidator& f, ParseNode* pos, Type* type) {
  ...
  Type actual;
  if (!CheckExpr(f, operand, &actual)) return false;
...
static bool CheckNot(FunctionValidator& f, ParseNode* expr, Type* type) {
  ...
  Type operandType;
  if (!CheckExpr(f, operand, &operandType)) return false;
...
static bool CheckBitNot(FunctionValidator& f, ParseNode* neg, Type* type) {
  ...
  if (!f.encoder().writeOp(MozOp::I32BitNot)) return false;
...
static bool CheckComma(FunctionValidator& f, ParseNode* comma, Type* type) {
  ...
  if (!f.encoder().writeOp(Op::Block)) return false;

  size_t typeAt;
  if (!f.encoder().writePatchableFixedU7(&typeAt)) return false;

  ParseNode* pn = operands;
  for (; NextNode(pn); pn = NextNode(pn)) {
    if (!CheckAsExprStatement(f, pn)) return false;
  }

  if (!CheckExpr(f, pn, type)) return false;
...
static bool CheckConditional(FunctionValidator& f, ParseNode* ternary,
  ...
  Type condType;
  if (!CheckExpr(f, cond, &condType)) return false;

  if (!condType.isInt())
    return f.failf(cond, "%s is not a subtype of int", condType.toChars());

  size_t typeAt;
  if (!f.pushIf(&typeAt)) return false;

  Type thenType;
  if (!CheckExpr(f, thenExpr, &thenType)) return false;

  if (!f.switchToElse()) return false;

  Type elseType;
  if (!CheckExpr(f, elseExpr, &elseType)) return false;
  ...
  if (!f.popIf(typeAt, type->toWasmBlockSignatureType())) return false;

  return true;
}

static bool IsValidIntMultiplyConstant(ModuleValidator& m, ParseNode* expr) {
  if (!IsNumericLiteral(m, expr)) return false;

  NumLit lit = ExtractNumericLiteral(m, expr);
  switch (lit.which()) {
    case NumLit::Fixnum:
    case NumLit::NegativeInt:
      if (abs(lit.toInt32()) < (1 << 20)) return true;
      return false;
    case NumLit::BigUnsigned:
    case NumLit::Double:
    case NumLit::Float:
...
static bool CheckAddOrSub(FunctionValidator& f, ParseNode* expr, Type* type,
  ...
  if (lhs->isKind(ParseNodeKind::Add) || lhs->isKind(ParseNodeKind::Sub)) {
    if (!CheckAddOrSub(f, lhs, &lhsType, &lhsNumAddOrSub)) return false;
    if (lhsType == Type::Intish) lhsType = Type::Int;
  } else {
    if (!CheckExpr(f, lhs, &lhsType)) return false;
    lhsNumAddOrSub = 0;
  }
  ...
  if (rhs->isKind(ParseNodeKind::Add) || rhs->isKind(ParseNodeKind::Sub)) {
    if (!CheckAddOrSub(f, rhs, &rhsType, &rhsNumAddOrSub)) return false;
    if (rhsType == Type::Intish) rhsType = Type::Int;
  } else {
    if (!CheckExpr(f, rhs, &rhsType)) return false;
    rhsNumAddOrSub = 0;
  }
  ...
  if (numAddOrSubOut) *numAddOrSubOut = numAddOrSub;
  return true;
}
...
static bool CheckAsExprStatement(FunctionValidator& f, ParseNode* expr) {
  ...
  Type resultType;
  if (!CheckExpr(f, expr, &resultType)) return false;
...
static bool CheckLoopConditionOnEntry(FunctionValidator& f, ParseNode* cond) {
  uint32_t maybeLit;
  if (IsLiteralInt(f.m(), cond, &maybeLit) && maybeLit) return true;
  ...
  if (!f.encoder().writeOp(Op::I32Eqz)) return false;

  // brIf (i32.eqz $f) $out
  if (!f.writeBreakIf()) return false;
...
static bool CheckWhile(FunctionValidator& f, ParseNode* whileStmt,
  ...
  //    )
  // )
  if (labels && !f.addLabels(*labels, 0, 1)) return false;

  if (!f.pushLoop()) return false;

  if (!CheckLoopConditionOnEntry(f, cond)) return false;
  if (!CheckStatement(f, body)) return false;
  if (!f.writeContinue()) return false;

  if (!f.popLoop()) return false;
  if (labels) f.removeLabels(*labels);
  return true;
}
...
static bool CheckFor(FunctionValidator& f, ParseNode* forStmt,
  ...
  if (labels && !f.addLabels(*labels, 1, 3)) return false;

  if (!f.pushUnbreakableBlock()) return false;

  if (maybeInit && !CheckAsExprStatement(f, maybeInit)) return false;

  {
    if (!f.pushLoop()) return false;

    if (maybeCond && !CheckLoopConditionOnEntry(f, maybeCond)) return false;

    {
      // Continuing in the body should just break out to the increment.
      if (!f.pushContinuableBlock()) return false;
      if (!CheckStatement(f, body)) return false;
      if (!f.popContinuableBlock()) return false;
    }

    if (maybeInc && !CheckAsExprStatement(f, maybeInc)) return false;

    if (!f.writeContinue()) return false;
    if (!f.popLoop()) return false;
  }

  if (!f.popUnbreakableBlock()) return false;
...
static bool CheckDoWhile(FunctionValidator& f, ParseNode* whileStmt,
  ...
  if (labels && !f.addLabels(*labels, 0, 2)) return false;
  ...
  if (!f.writeContinueIf()) return false;
...
static bool CheckLabel(FunctionValidator& f, ParseNode* labeledStmt) {
  ...
  do {
    if (!labels.append(LabeledStatementLabel(innermost))) return false;
  ...
  if (!f.pushUnbreakableBlock(&labels)) return false;

  if (!CheckStatement(f, innermost)) return false;

  if (!f.popUnbreakableBlock(&labels)) return false;
  return true;
}
...
recurse:
  ...
  if (!CheckStatement(f, thenStmt)) return false;
  ...
  if (elseStmt) {
    ...
    if (elseStmt->isKind(ParseNodeKind::If)) {
      ifStmt = elseStmt;
      if (numIfEnd++ == UINT32_MAX) return false;
      goto recurse;
    }

    if (!CheckStatement(f, elseStmt)) return false;
  }

  for (uint32_t i = 0; i != numIfEnd; ++i) {
    if (!f.popIf()) return false;
  }
...
static bool CheckSwitchRange(FunctionValidator& f, ParseNode* stmt,
  ...
  int32_t i = 0;
  if (!CheckCaseExpr(f, CaseExpr(stmt), &i)) return false;
...
static bool CheckSwitchExpr(FunctionValidator& f, ParseNode* switchExpr) {
  Type exprType;
  if (!CheckExpr(f, switchExpr, &exprType)) return false;
...
static bool CheckSwitch(FunctionValidator& f, ParseNode* switchStmt) {
  ...
  if (!stmt) {
    if (!CheckSwitchExpr(f, switchExpr)) return false;
    if (!f.encoder().writeOp(Op::Drop)) return false;
    return true;
  }

  if (!CheckDefaultAtEnd(f, stmt)) return false;

  int32_t low = 0, high = 0;
  uint32_t tableLength = 0;
  if (!CheckSwitchRange(f, stmt, &low, &high, &tableLength)) return false;

  static const uint32_t CASE_NOT_DEFINED = UINT32_MAX;

  Uint32Vector caseDepths;
  if (!caseDepths.appendN(CASE_NOT_DEFINED, tableLength)) return false;
  ...
  if (!f.pushBreakableBlock()) return false;
  ...
  if (low) {
    if (!CheckSwitchExpr(f, switchExpr)) return false;
    if (!f.writeInt32Lit(low)) return false;
    if (!f.encoder().writeOp(Op::I32Sub)) return false;
  } else {
    if (!CheckSwitchExpr(f, switchExpr)) return false;
  }

  // Start the br_table block.
  if (!f.encoder().writeOp(Op::BrTable)) return false;

  // Write the number of cases (tableLength - 1 + 1 (default)).
  // Write the number of cases (tableLength - 1 + 1 (default)).
  if (!f.encoder().writeVarU32(tableLength)) return false;
  ...
  for (size_t i = 0; i < tableLength; i++) {
    ...
    if (!f.encoder().writeVarU32(target)) return false;
  }

  // Write the default depth.
  if (!f.encoder().writeVarU32(defaultDepth)) return false;

  // Our br_table is done. Close its block, write the cases down in order.
  if (!f.popUnbreakableBlock()) return false;

  for (; stmt && !IsDefaultCase(stmt); stmt = NextNode(stmt)) {
    if (!CheckStatement(f, CaseBody(stmt))) return false;
    if (!f.popUnbreakableBlock()) return false;
  }
  ...
  if (!f.popBreakableBlock()) return false;
  return true;
}
...
static bool CheckReturn(FunctionValidator& f, ParseNode* returnStmt) {
  ...
  if (!expr) {
    if (!CheckReturnType(f, returnStmt, Type::Void)) return false;
  } else {
    Type type;
    if (!CheckExpr(f, expr, &type)) return false;

    if (!type.isReturnType())
      return f.failf(expr, "%s is not a valid return type", type.toChars());

    if (!CheckReturnType(f, expr, Type::canonicalize(type))) return false;
  }

  if (!f.encoder().writeOp(Op::Return)) return false;
...
static bool CheckStatementList(FunctionValidator& f, ParseNode* stmtList,
  ...
  for (ParseNode* stmt = ListHead(stmtList); stmt; stmt = NextNode(stmt)) {
    if (!CheckStatement(f, stmt)) return false;
  }
...
static bool CheckLexicalScope(FunctionValidator& f, ParseNode* lexicalScope) {
...
static bool ParseFunction(ModuleValidator& m, ParseNode** fnOut,
                          unsigned* line) {
  ...
  *line = anyChars.srcCoords.lineNum(anyChars.currentToken().pos.end);
  ...
  TokenKind tk;
  if (!tokenStream.getToken(&tk, TokenStreamShared::Operand)) return false;
  if (tk == TokenKind::Mul)
  ...
  FunctionBox* funbox = m.parser().newFunctionBox(
      fn, fun, toStringStart, directives, GeneratorKind::NotGenerator,
  ...
  if (!funpc.init()) return false;

  if (!m.parser().functionFormalParametersAndBody(InAllowed, YieldIsName, &fn,
                                                  Statement)) {
    if (anyChars.hadError() || directives == newDirectives) return false;
...
static bool CheckFunction(ModuleValidator& m) {
  ...
  unsigned line = 0;
  if (!ParseFunction(m, &fn, &line)) return false;

  if (!CheckFunctionHead(m, fn)) return false;
  ...
  if (!CheckProcessingDirectives(m, &stmtIter)) return false;

  ValTypeVector args;
  if (!CheckArguments(f, &stmtIter, &args)) return false;

  if (!CheckVariables(f, &stmtIter)) return false;

  ParseNode* lastNonEmptyStmt = nullptr;
  for (; stmtIter; stmtIter = NextNonEmptyStatement(stmtIter)) {
    lastNonEmptyStmt = stmtIter;
    if (!CheckStatement(f, stmtIter)) return false;
  }

  if (!CheckFinalReturn(f, lastNonEmptyStmt)) return false;

  ModuleValidator::Func* func = nullptr;
  if (!CheckFunctionSignature(m, fn, Sig(Move(args), f.returnedType()),
...
static bool CheckFunctions(ModuleValidator& m) {
  while (true) {
    TokenKind tk;
    if (!PeekToken(m.parser(), &tk)) return false;
...
static bool CheckFuncPtrTable(ModuleValidator& m, ParseNode* var) {
  ...
  for (ParseNode* elem = ListHead(arrayLiteral); elem; elem = NextNode(elem)) {
    ...
    if (!elemFuncDefIndices.append(func->funcDefIndex())) return false;
  }

  Sig copy;
  if (!copy.clone(*sig)) return false;

  uint32_t tableIndex;
  if (!CheckFuncPtrTableAgainstExisting(m, var, var->name(), Move(copy), mask,
                                        &tableIndex))
    return false;
...
static bool CheckFuncPtrTables(ModuleValidator& m) {
  while (true) {
    ...
    if (!varStmt) break;
    for (ParseNode* var = VarListHead(varStmt); var; var = NextNode(var)) {
      if (!CheckFuncPtrTable(m, var)) return false;
    }
  }
...
static bool CheckModuleExportFunction(ModuleValidator& m, ParseNode* pn,
  ...
  const ModuleValidator::Func* func = m.lookupFuncDef(funcName);
  if (!func) return m.failName(pn, "function '%s' not found", funcName);
...
static bool CheckModuleExportObject(ModuleValidator& m, ParseNode* object) {
  ...
  for (ParseNode* pn = ListHead(object); pn; pn = NextNode(pn)) {
    ...
    if (!CheckModuleExportFunction(m, initNode, fieldName)) return false;
  }
...
static bool CheckModuleReturn(ModuleValidator& m) {
  TokenKind tk;
  if (!GetToken(m.parser(), &tk)) return false;
  auto& ts = m.parser().tokenStream;
  ...
  if (returnExpr->isKind(ParseNodeKind::Object)) {
    if (!CheckModuleExportObject(m, returnExpr)) return false;
  } else {
    if (!CheckModuleExportFunction(m, returnExpr)) return false;
  }
...
static SharedModule CheckModule(JSContext* cx, AsmJSParser& parser,
  ...
  if (!CheckFunctionHead(m, moduleFunctionNode)) return nullptr;

  if (!CheckModuleArguments(m, moduleFunctionNode)) return nullptr;

  if (!CheckPrecedingStatements(m, stmtList)) return nullptr;

  if (!CheckModuleProcessingDirectives(m)) return nullptr;

  if (!CheckModuleGlobals(m)) return nullptr;

  if (!m.startFunctionBodies()) return nullptr;

  if (!CheckFunctions(m)) return nullptr;

  if (!CheckFuncPtrTables(m)) return nullptr;

  if (!CheckModuleReturn(m)) return nullptr;

  if (!CheckModuleEnd(m)) return nullptr;
...
static bool GetDataProperty(JSContext* cx, HandleValue objVal, HandleAtom field,
  ...
  if (!GetPropertyDescriptor(cx, obj, id, &desc)) return false;

  if (!desc.object()) return LinkFail(cx, "property not present on object");
...
static bool HasObjectValueOfMethodPure(JSObject* obj, JSContext* cx) {
  ...
  JSFunction* fun;
  if (!IsFunctionObject(v, &fun)) return false;
...
static bool HasPureCoercion(JSContext* cx, HandleValue v) {
  ...
  if (IsVectorObject<Int32x4>(v) || IsVectorObject<Int16x8>(v) ||
      IsVectorObject<Int8x16>(v) || IsVectorObject<Bool32x4>(v) ||
      IsVectorObject<Bool16x8>(v) || IsVectorObject<Bool8x16>(v) ||
      IsVectorObject<Float32x4>(v)) {
    return true;
  }
...
static bool ValidateGlobalVariable(JSContext* cx, const AsmJSGlobal& global,
  ...
  switch (global.varInitKind()) {
    ...
    case AsmJSGlobal::InitImport: {
      RootedValue v(cx);
      if (!GetDataProperty(cx, importVal, global.field(), &v)) return false;
      ...
      switch (global.varInitImportType()) {
        case ValType::I32: {
          int32_t i32;
          if (!ToInt32(cx, v, &i32)) return false;
        ...
        case ValType::F32: {
          float f;
          if (!RoundFloat32(cx, v, &f)) return false;
          *val = Val(f);
          return true;
        }
        case ValType::F64: {
        ...
        case ValType::I8x16: {
          ...
          if (!ToSimdConstant<Int8x16>(cx, v, &simdConstant)) return false;
          *val = Val(simdConstant.asInt8x16());
          return true;
        }
        case ValType::I16x8: {
          SimdConstant simdConstant;
          if (!ToSimdConstant<Int16x8>(cx, v, &simdConstant)) return false;
          *val = Val(simdConstant.asInt16x8());
          return true;
        }
        case ValType::I32x4: {
          SimdConstant simdConstant;
          if (!ToSimdConstant<Int32x4>(cx, v, &simdConstant)) return false;
          *val = Val(simdConstant.asInt32x4());
          return true;
        }
        case ValType::F32x4: {
          SimdConstant simdConstant;
          if (!ToSimdConstant<Float32x4>(cx, v, &simdConstant)) return false;
          *val = Val(simdConstant.asFloat32x4());
          return true;
        }
        case ValType::B8x16: {
          SimdConstant simdConstant;
          if (!ToSimdConstant<Bool8x16>(cx, v, &simdConstant)) return false;
        ...
        case ValType::B16x8: {
          SimdConstant simdConstant;
          if (!ToSimdConstant<Bool16x8>(cx, v, &simdConstant)) return false;
        ...
        case ValType::B32x4: {
          SimdConstant simdConstant;
          if (!ToSimdConstant<Bool32x4>(cx, v, &simdConstant)) return false;
...
static bool ValidateArrayView(JSContext* cx, const AsmJSGlobal& global,
  ...
  if (!global.field()) return true;

  RootedValue v(cx);
  if (!GetDataProperty(cx, globalVal, global.field(), &v)) return false;

  bool tac = IsTypedArrayConstructor(v, global.viewType());
  if (!tac) return LinkFail(cx, "bad typed array constructor");
...
static bool ValidateMathBuiltinFunction(JSContext* cx,
                                        ...
                                        HandleValue globalVal) {
  RootedValue v(cx);
  if (!GetDataProperty(cx, globalVal, cx->names().Math, &v)) return false;

  if (!GetDataProperty(cx, v, global.field(), &v)) return false;
...
static bool ValidateSimdType(JSContext* cx, const AsmJSGlobal& global,
                             HandleValue globalVal, MutableHandleValue out) {
  RootedValue v(cx);
  if (!GetDataProperty(cx, globalVal, cx->names().SIMD, &v)) return false;
  ...
  if (!GetDataProperty(cx, v, simdTypeName, &v)) return false;

  if (!v.isObject()) return LinkFail(cx, "bad SIMD type");

  RootedObject simdDesc(cx, &v.toObject());
  if (!simdDesc->is<SimdTypeDescr>()) return LinkFail(cx, "bad SIMD type");
...
static bool ValidateSimdOperation(JSContext* cx, const AsmJSGlobal& global,
                                  HandleValue globalVal) {
  RootedValue v(cx);
...
#define SET_NATIVE_INT8X16(op)  \
  case SimdOperation::Fn_##op:  \
    native = simd_int8x16_##op; \
    break;
#define SET_NATIVE_INT16X8(op)  \
  case SimdOperation::Fn_##op:  \
    native = simd_int16x8_##op; \
    break;
#define SET_NATIVE_INT32X4(op)  \
  case SimdOperation::Fn_##op:  \
    native = simd_int32x4_##op; \
    break;
#define SET_NATIVE_UINT8X16(op)  \
  case SimdOperation::Fn_##op:   \
    native = simd_uint8x16_##op; \
    break;
#define SET_NATIVE_UINT16X8(op)  \
  case SimdOperation::Fn_##op:   \
    native = simd_uint16x8_##op; \
    break;
#define SET_NATIVE_UINT32X4(op)  \
  case SimdOperation::Fn_##op:   \
    native = simd_uint32x4_##op; \
    break;
#define SET_NATIVE_FLOAT32X4(op)  \
  case SimdOperation::Fn_##op:    \
    native = simd_float32x4_##op; \
    break;
#define SET_NATIVE_BOOL8X16(op)  \
  case SimdOperation::Fn_##op:   \
    native = simd_bool8x16_##op; \
    break;
#define SET_NATIVE_BOOL16X8(op)  \
  case SimdOperation::Fn_##op:   \
    native = simd_bool16x8_##op; \
    break;
#define SET_NATIVE_BOOL32X4(op)  \
  case SimdOperation::Fn_##op:   \
    native = simd_bool32x4_##op; \
    break;
#define FALLTHROUGH(op) case SimdOperation::Fn_##op:
    case SimdType::Int8x16:
      switch (global.simdOperation()) {
        ...
        default:
          MOZ_CRASH("shouldn't have been validated in the first place");
      }
      break;
    case SimdType::Int16x8:
      switch (global.simdOperation()) {
...
static bool ValidateAtomicsBuiltinFunction(JSContext* cx,
                                           const AsmJSGlobal& global,
                                           HandleValue globalVal) {
  RootedValue v(cx);
  if (!GetDataProperty(cx, globalVal, cx->names().Atomics, &v)) return false;
  ...
  switch (global.atomicsBuiltinFunction()) {
    case AsmJSAtomicsBuiltin_compareExchange:
      native = atomics_compareExchange;
      break;
    case AsmJSAtomicsBuiltin_exchange:
      native = atomics_exchange;
      break;
    case AsmJSAtomicsBuiltin_load:
      native = atomics_load;
      break;
    case AsmJSAtomicsBuiltin_store:
      native = atomics_store;
      break;
    case AsmJSAtomicsBuiltin_add:
      native = atomics_add;
      break;
...
static bool ValidateConstant(JSContext* cx, const AsmJSGlobal& global,
  ...
  if (global.constantKind() == AsmJSGlobal::MathConstant) {
    if (!GetDataProperty(cx, v, cx->names().Math, &v)) return false;
  }
  ...
  // allows unaligned memory access (see MaxMemoryAccessSize comment);
    ...
    if (!buffer->as<SharedArrayBufferObject>().isPreparedForAsmJS()) {
      if (buffer->as<SharedArrayBufferObject>().isWasm())
        return LinkFail(
            cx, "SharedArrayBuffer created for Wasm cannot be used for asm.js");
      if (!jit::JitOptions.asmJSAtomicsEnable)
        return LinkFail(cx,
                        "Can link with SharedArrayBuffer only when "
                        "asmjs.atomics.enable is set");
...
static bool GetImports(JSContext* cx, const AsmJSMetadata& metadata,
  ...
  if (!ffis.resize(metadata.numFFIs)) return false;

  for (const AsmJSGlobal& global : metadata.asmJSGlobals) {
    switch (global.which()) {
      case AsmJSGlobal::Variable: {
        Val val;
        if (!ValidateGlobalVariable(cx, global, importVal, &val)) return false;
        if (!valImports->append(val)) return false;
        break;
      }
      case AsmJSGlobal::FFI:
        if (!ValidateFFI(cx, global, importVal, &ffis)) return false;
        break;
      case AsmJSGlobal::ArrayView:
      case AsmJSGlobal::ArrayViewCtor:
        if (!ValidateArrayView(cx, global, globalVal)) return false;
        break;
      case AsmJSGlobal::MathBuiltinFunction:
        if (!ValidateMathBuiltinFunction(cx, global, globalVal)) return false;
        break;
      ...
      case AsmJSGlobal::Constant:
        if (!ValidateConstant(cx, global, globalVal)) return false;
        break;
      case AsmJSGlobal::SimdCtor:
        if (!ValidateSimdType(cx, global, globalVal)) return false;
        break;
      case AsmJSGlobal::SimdOp:
        if (!ValidateSimdOperation(cx, global, globalVal)) return false;
        break;
    }
  }

  for (const AsmJSImport& import : metadata.asmJSImports) {
    if (!funcImports.append(ffis[import.ffiIndex()])) return false;
  }

  return true;
}

static bool TryInstantiate(JSContext* cx, CallArgs args, Module& module,
  ...
  if (module.metadata().usesMemory()) {
    if (!CheckBuffer(cx, metadata, bufferVal, &buffer)) return false;
  ...
  if (!module.instantiate(cx, funcs, table, memory, valImports, nullptr,
                          instanceObj))
    return false;
...
static bool HandleInstantiationFailure(JSContext* cx, CallArgs args,
  ...
  if (metadata.strict) options.strictOption = true;

  AutoStableStringChars stableChars(cx);
  if (!stableChars.initTwoByte(cx, src)) return false;

  const char16_t* chars = stableChars.twoByteRange().begin().get();
  SourceBufferHolder::Ownership ownership =
      stableChars.maybeGiveOwnershipToCaller()
...
static Module& AsmJSModuleFunctionToModule(JSFunction* fun) {
...
class ModuleCharsForStore : ModuleChars {
 ...
 public:
  bool init(AsmJSParser& parser) {
    ...
    if (maxCompressedSize < uncompressedSize_) return false;

    if (!compressedBuffer_.resize(maxCompressedSize)) return false;

    const char16_t* chars =
        parser.tokenStream.rawCharPtrAt(beginOffset(parser));
    const char* source = reinterpret_cast<const char*>(chars);
    size_t compressedSize =
        LZ4::compress(source, uncompressedSize_, compressedBuffer_.begin());
    if (!compressedSize || compressedSize > UINT32_MAX) return false;
    ...
    if (isFunCtor_) {
      ...
      for (unsigned i = 0; i < numArgs; i++, arg = arg->pn_next) {
        ...
        if (!name || !funCtorArgs_.append(Move(name))) return false;
      }
    }
  ...
  uint8_t* serialize(uint8_t* cursor) const {
    ...
    if (isFunCtor_) cursor = SerializeVector(cursor, funCtorArgs_);
    return cursor;
  }
};
...
class ModuleCharsForLookup : ModuleChars {
 ...
 public:
  const uint8_t* deserialize(const uint8_t* cursor) {
    ...
    if (!chars_.resize(uncompressedSize / sizeof(char16_t))) return nullptr;

    const char* source = reinterpret_cast<const char*>(cursor);
    char* dest = reinterpret_cast<char*>(chars_.begin());
    if (!LZ4::decompress(source, dest, uncompressedSize)) return nullptr;

    cursor += compressedSize;

    cursor = ReadScalar<uint32_t>(cursor, &isFunCtor_);
    if (isFunCtor_) cursor = DeserializeVector(cursor, &funCtorArgs_);
  ...
  bool match(AsmJSParser& parser) const {
    ...
    if (uint32_t(parseLimit - parseBegin) < chars_.length()) return false;
    if (!PodEqual(chars_.begin(), parseBegin, chars_.length())) return false;
    if (isFunCtor_ != parser.pc->isStandaloneFunctionBody()) return false;
    if (isFunCtor_) {
      ...
      if (parseBegin + chars_.length() != parseLimit) return false;
      unsigned numArgs;
      ParseNode* functionNode = parser.pc->functionBox()->functionNode;
      ParseNode* arg = FunctionFormalParametersList(functionNode, &numArgs);
      if (funCtorArgs_.length() != numArgs) return false;
      for (unsigned i = 0; i < funCtorArgs_.length(); i++, arg = arg->pn_next) {
        UniqueChars name = StringToNewUTF8CharsZ(nullptr, *arg->name());
        if (!name || strcmp(funCtorArgs_[i].get(), name.get())) return false;
      }
    }
    return true;
  }
...
static JS::AsmJSCacheResult StoreAsmJSModuleInCache(AsmJSParser& parser,
  ...
  if (!moduleChars.init(parser)) return JS::AsmJSCache_InternalError;
  ...
  size_t serializedSize =
      sizeof(uint32_t) + compiledSize + moduleChars.serializedSize();

  JS::OpenAsmJSCacheEntryForWriteOp open =
      cx->asmJSCacheOps().openEntryForWrite;
  if (!open) return JS::AsmJSCache_Disabled_Internal;

  const char16_t* begin =
      parser.tokenStream.rawCharPtrAt(ModuleChars::beginOffset(parser));
  const char16_t* end =
      parser.tokenStream.rawCharPtrAt(ModuleChars::endOffset(parser));

  ScopedCacheEntryOpenedForWrite entry(cx, serializedSize);
  JS::AsmJSCacheResult openResult = open(
      cx->global(), begin, end, serializedSize, &entry.memory, &entry.handle);
  if (openResult != JS::AsmJSCache_Success) return openResult;
...
static bool LookupAsmJSModuleInCache(JSContext* cx, AsmJSParser& parser,
  ...
  *loadedFromCache = false;
  ...
  if (!assumptions.initBuildIdFromContext(cx)) return false;

  if (!Module::assumptionsMatch(assumptions, cursor, remain)) return true;

  MutableAsmJSMetadata asmJSMetadata = cx->new_<AsmJSMetadata>();
  if (!asmJSMetadata) return false;

  *module =
      Module::deserialize(/* bytecodeBegin = */ nullptr, /* bytecodeSize = */ 0,
  ...
  if (!moduleChars.match(parser)) return true;
...
#endif
  if (cursor != entry.memory + entry.serializedSize) return true;
  ...
  asmJSMetadata->srcStart =
      parser.pc->functionBox()->functionNode->pn_body->pn_pos.begin;
  ...
  *compilationTimeReport = JS_smprintf("loaded from cache in %dms", ms);
  if (!*compilationTimeReport) return false;
...
bool js::CompileAsmJS(JSContext* cx, AsmJSParser& parser, ParseNode* stmtList,
  ...
  *validated = false;
  ...
  if (!EstablishPreconditions(cx, parser)) return NoExceptionPending(cx);
  ...
  if (!loadedFromCache) {
    ...
    if (!module) return NoExceptionPending(cx);
...
static JSFunction* MaybeWrappedNativeFunction(const Value& v) {
  ...
  if (!obj->is<JSFunction>()) return nullptr;
...
JSString* js::AsmJSModuleToString(JSContext* cx, HandleFunction fun,
  ...
  if (isToSource && fun->isLambda() && !out.append("(")) return nullptr;
  ...
  if (!haveSource) {
    if (!out.append("function ")) return nullptr;
    if (fun->explicitName() && !out.append(fun->explicitName())) return nullptr;
    if (!out.append("() {\n    [sourceless code]\n}")) return nullptr;
  } else {
    Rooted<JSFlatString*> src(cx, source->substring(cx, begin, end));
    if (!src) return nullptr;

    if (!out.append(src)) return nullptr;
  }
...
JSString* js::AsmJSFunctionToString(JSContext* cx, HandleFunction fun) {
  ...
  if (!haveSource) {
    ...
    if (!out.append(fun->explicitName())) return nullptr;
    if (!out.append("() {\n    [sourceless code]\n}")) return nullptr;
  } else {
    Rooted<JSFlatString*> src(cx, source->substring(cx, begin, end));
    if (!src) return nullptr;
...
bool js::IsValidAsmJSHeapLength(uint32_t length) {
  if (length < MinHeapLength) return false;
