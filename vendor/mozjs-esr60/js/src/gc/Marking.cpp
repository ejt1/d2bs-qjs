...
static inline bool IsThingPoisoned(T* thing) {
  const uint8_t poisonBytes[] = {JS_FRESH_NURSERY_PATTERN,     JS_SWEPT_NURSERY_PATTERN, JS_ALLOCATED_NURSERY_PATTERN,
                                 JS_FRESH_TENURED_PATTERN,     JS_MOVED_TENURED_PATTERN, JS_SWEPT_TENURED_PATTERN,
                                 JS_ALLOCATED_TENURED_PATTERN, JS_SWEPT_CODE_PATTERN};
...
static bool IsMovingTracer(JSTracer* trc) {
  return trc->isCallbackTracer() && trc->asCallbackTracer()->getTracerKind() == JS::CallbackTracer::TracerKind::Moving;
}
#endif
...
static inline bool IsOwnedByOtherRuntime(JSRuntime* rt, T thing) {
  ...
  MOZ_ASSERT_IF(other, ThingIsPermanentAtomOrWellKnownSymbol(thing) || thing->zoneFromAnyThread()->isSelfHostingZone());
  return other;
}
...
void js::CheckTracedThing(JSTracer* trc, T* thing) {
  ...
  MOZ_ASSERT(MapTypeToTraceKind<typename mozilla::RemovePointer<T>::Type>::kind == thing->getTraceKind());
  ...
  MOZ_ASSERT_IF(zone->requireGCTracer(), isGcMarkingTracer || IsBufferGrayRootsTracer(trc) || IsUnmarkGrayTracer(trc));

  if (isGcMarkingTracer) {
    GCMarker* gcMarker = GCMarker::fromTracer(trc);
    MOZ_ASSERT_IF(gcMarker->shouldCheckCompartments(), zone->isCollecting() || zone->isAtomsZone());

    MOZ_ASSERT_IF(gcMarker->markColor() == MarkColor::Gray, !zone->isGCMarkingBlack() || zone->isAtomsZone());
   ...
   */
  MOZ_ASSERT_IF(JS::CurrentThreadIsHeapBusy() && !zone->isGCCompacting() && !rt->gc.isBackgroundSweeping(),
...
namespace js {
#define IMPL_CHECK_TRACED_THING(_, type, __) template void CheckTracedThing<type>(JSTracer*, type*);
...
static bool ShouldTraceCrossCompartment(JSTracer* trc, JSObject* src, Cell* cell) {
...
static bool ShouldTraceCrossCompartment(JSTracer* trc, JSObject* src, const Value& val) {
...
static void AssertRootMarkingPhase(JSTracer* trc) {
  MOZ_ASSERT_IF(trc->isMarkingTracer(),
                trc->runtime()->gc.state() == State::NotActive || trc->runtime()->gc.state() == State::MarkRoots);
}
...
#define EXPAND_MATCH_TYPE(name, type, _) IsBaseOf<type, T>::value ? JS::TraceKind::name:
...
#define IMPL_BASE_GC_TYPE(name, type_, _)     \
  template <typename T>                       \
  struct BaseGCType<T, JS::TraceKind::name> { \
    typedef type_ type;                       \
  };
...
struct PtrBaseGCType<T*> {
  typedef typename BaseGCType<T>::type* type;
};
...
template <typename T>
void DispatchToTracer(JSTracer* trc, T* thingp, const char* name);
template <typename T>
...
template <typename T>
void js::TraceEdge(JSTracer* trc, WriteBarrieredBase<T>* thingp, const char* name) {
  DispatchToTracer(trc, ConvertToBase(thingp->unsafeUnbarrieredForTracing()), name);
}

template <typename T>
void js::TraceEdge(JSTracer* trc, ReadBarriered<T>* thingp, const char* name) {
  DispatchToTracer(trc, ConvertToBase(thingp->unsafeGet()), name);
}

template <typename T>
void js::TraceNullableEdge(JSTracer* trc, WriteBarrieredBase<T>* thingp, const char* name) {
  if (InternalBarrierMethods<T>::isMarkable(thingp->get()))
    DispatchToTracer(trc, ConvertToBase(thingp->unsafeUnbarrieredForTracing()), name);
}

template <typename T>
void js::TraceNullableEdge(JSTracer* trc, ReadBarriered<T>* thingp, const char* name) {
  if (InternalBarrierMethods<T>::isMarkable(thingp->unbarrieredGet()))
    DispatchToTracer(trc, ConvertToBase(thingp->unsafeGet()), name);
}

template <typename T>
JS_PUBLIC_API void js::gc::TraceExternalEdge(JSTracer* trc, T* thingp, const char* name) {
  MOZ_ASSERT(InternalBarrierMethods<T>::isMarkable(*thingp));
  DispatchToTracer(trc, ConvertToBase(thingp), name);
}

template <typename T>
void js::TraceManuallyBarrieredEdge(JSTracer* trc, T* thingp, const char* name) {
  DispatchToTracer(trc, ConvertToBase(thingp), name);
}

template <typename T>
JS_PUBLIC_API void js::UnsafeTraceManuallyBarrieredEdge(JSTracer* trc, T* thingp, const char* name) {
  DispatchToTracer(trc, ConvertToBase(thingp), name);
}

template <typename T>
void js::TraceWeakEdge(JSTracer* trc, WeakRef<T>* thingp, const char* name) {
  if (!trc->isMarkingTracer()) {
  ...
  NoteWeakEdge(GCMarker::fromTracer(trc), ConvertToBase(thingp->unsafeUnbarrieredForTracing()));
}

template <typename T>
void js::TraceRoot(JSTracer* trc, T* thingp, const char* name) {
  AssertRootMarkingPhase(trc);
  DispatchToTracer(trc, ConvertToBase(thingp), name);
}

template <typename T>
void js::TraceRoot(JSTracer* trc, ReadBarriered<T>* thingp, const char* name) {
  TraceRoot(trc, thingp->unsafeGet(), name);
}

template <typename T>
void js::TraceNullableRoot(JSTracer* trc, T* thingp, const char* name) {
  AssertRootMarkingPhase(trc);
...
template <typename T>
void js::TraceNullableRoot(JSTracer* trc, ReadBarriered<T>* thingp, const char* name) {
  TraceNullableRoot(trc, thingp->unsafeGet(), name);
}

template <typename T>
JS_PUBLIC_API void JS::UnsafeTraceRoot(JSTracer* trc, T* thingp, const char* name) {
  MOZ_ASSERT(thingp);
  js::TraceNullableRoot(trc, thingp, name);
}

template <typename T>
void js::TraceRange(JSTracer* trc, size_t len, WriteBarrieredBase<T>* vec, const char* name) {
...
template <typename T>
void js::TraceRootRange(JSTracer* trc, size_t len, T* vec, const char* name) {
  AssertRootMarkingPhase(trc);
  JS::AutoTracingIndex index(trc);
  for (auto i : IntegerRange(len)) {
...
#define INSTANTIATE_PUBLIC_TRACE_FUNCTIONS(type)                                                         \
  ...
  template JS_PUBLIC_API void js::UnsafeTraceManuallyBarrieredEdge<type>(JSTracer*, type*, const char*); \
...
template <typename T>
void js::TraceManuallyBarrieredCrossCompartmentEdge(JSTracer* trc, JSObject* src, T* dst, const char* name) {
  if (ShouldTraceCrossCompartment(trc, src, *dst))
    DispatchToTracer(trc, dst, name);
}
template void js::TraceManuallyBarrieredCrossCompartmentEdge<JSObject*>(JSTracer*, JSObject*, JSObject**, const char*);
template void js::TraceManuallyBarrieredCrossCompartmentEdge<JSScript*>(JSTracer*, JSObject*, JSScript**, const char*);

template <typename T>
void js::TraceCrossCompartmentEdge(JSTracer* trc, JSObject* src, WriteBarrieredBase<T>* dst, const char* name) {
  if (ShouldTraceCrossCompartment(trc, src, dst->get()))
    DispatchToTracer(trc, dst->unsafeUnbarrieredForTracing(), name);
}
template void js::TraceCrossCompartmentEdge<Value>(JSTracer*, JSObject*, WriteBarrieredBase<Value>*, const char*);
...
void js::TraceGenericPointerRoot(JSTracer* trc, Cell** thingp, const char* name) {
  MOZ_ASSERT(thingp);
  if (!*thingp)
    return;
  TraceRootFunctor f;
...
void js::TraceManuallyBarrieredGenericPointerEdge(JSTracer* trc, Cell** thingp, const char* name) {
  MOZ_ASSERT(thingp);
  if (!*thingp)
    return;
...
#define IS_SAME_TYPE_OR(name, type, _) mozilla::IsSame<type*, T>::value ||
  static_assert(JS_FOR_EACH_TRACEKIND(IS_SAME_TYPE_OR) mozilla::IsSame<T, JS::Value>::value ||
                  mozilla::IsSame<T, jsid>::value || mozilla::IsSame<T, TaggedProto>::value,
...
void GCMarker::markEphemeronValues(gc::Cell* markedCell, WeakEntryVector& values) {
...
void js::GCMarker::noteWeakEdge(T* edge) {
  static_assert(IsBaseOf<Cell, typename mozilla::RemovePointer<T>::Type>::value, "edge must point to a GC pointer");
...
static void CheckTraversedEdge(S source, T* target) {
  ...
  // atom bitmap.
  MOZ_ASSERT_IF(
    !ThingIsPermanentAtomOrWellKnownSymbol(target) && target->zone()->isAtomsZone() && !source->zone()->isAtomsZone(),
    target->runtimeFromAnyThread()->gc.atomMarking.atomIsMarked(source->zone(),
                                                                reinterpret_cast<TenuredCell*>(target)));
...
inline void js::GCMarker::eagerlyMarkChildren(JSRope* rope) {
  ...
  while (true) {
    ...
    if (++ropeDepth >= DEEP_ROPE_THRESHOLD) {
      ...
      if ((ropeDepth > DEEP_ROPE_THRESHOLD + ROPE_CYCLE_HISTORY) && (ropeDepth % ROPE_CYCLE_HISTORY) == 0) {
...
static inline void TraceBindingNames(JSTracer* trc, BindingName* names, uint32_t length) {
...
static inline void TraceNullableBindingNames(JSTracer* trc, BindingName* names, uint32_t length) {
  for (uint32_t i = 0; i < length; i++) {
...
static inline NativeObject* CallTraceHook(Functor f, JSTracer* trc, JSObject* obj, CheckGeneration check,
                                          Args&&... args) {
  ...
  if (clasp->isTrace(InlineTypedObject::obj_trace)) {
    ...
    if (tobj.typeDescr().hasTraceList()) {
      VisitTraceList(f, tobj.typeDescr().traceList(), tobj.inlineTypedMemForGC(), mozilla::Forward<Args>(args)...);
    }
  ...
  if (clasp == &UnboxedPlainObject::class_) {
    ...
    const UnboxedLayout& layout =
      check == CheckGeneration::DoChecks ? unboxed.layout() : unboxed.layoutDontCheckGeneration();
    if (layout.traceList()) {
      VisitTraceList(f, layout.traceList(), unboxed.data(), mozilla::Forward<Args>(args)...);
    }
...
static void VisitTraceList(F f, const int32_t* traceList, uint8_t* memory, Args&&... args) {
  while (*traceList != -1) {
...
bool GCMarker::drainMarkStack(SliceBudget& budget) {
#ifdef DEBUG
    ...
    if (tag == MarkStack::ValueArrayTag) {
      ...
      if (array.end == vp + obj->getDenseInitializedLength()) {
        ...
        if (array.start == array.end) {
          ...
          MOZ_ASSERT(array.start >= obj->slots_ && array.end == obj->slots_ + obj->slotSpan() - nfixed);
...
bool GCMarker::restoreValueArray(const MarkStack::SavedValueArray& array, HeapSlot** vpp, HeapSlot** endp) {
...
static_assert(sizeof(MarkStack::TaggedPtr) == sizeof(uintptr_t), "A TaggedPtr should be the same size as a pointer");
...
static inline void CheckValueArray(const MarkStack::ValueArray& array) {
...
static inline void CheckSavedValueArray(const MarkStack::SavedValueArray& array) {
  MOZ_ASSERT(array.ptr.tag() == MarkStack::SavedValueArrayTag);
  MOZ_ASSERT(array.kind == HeapSlot::Slot || array.kind == HeapSlot::Element);
}

inline MarkStack::TaggedPtr::TaggedPtr(Tag tag, Cell* ptr) : bits(tag | uintptr_t(ptr)) {
...
inline MarkStack::ValueArray::ValueArray(JSObject* obj, HeapSlot* startArg, HeapSlot* endArg)
    : end(endArg), start(startArg), ptr(ValueArrayTag, obj) {}

inline MarkStack::SavedValueArray::SavedValueArray(JSObject* obj, size_t indexArg, HeapSlot::Kind kindArg)
...
inline void MarkStack::setStack(TaggedPtr* stack, size_t tosIndex, size_t capacity) {
  MOZ_ASSERT(iteratorCount_ == 0);
  stack_ = stack;
  tos_ = stack + tosIndex;
  end_ = stack + capacity;
...
void MarkStack::setBaseCapacity(JSGCMode mode) {
  switch (mode) {
...
size_t MarkStack::sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) const {
  return mallocSizeOf(stack_);
}

MarkStackIter::MarkStackIter(const MarkStack& stack) : stack_(stack), pos_(stack.tos_) {
#ifdef DEBUG
...
inline size_t MarkStackIter::position() const {
  return pos_ - stack_.stack_;
}

inline bool MarkStackIter::done() const {
  return position() == 0;
}
...
inline MarkStack::Tag MarkStackIter::peekTag() const {
  return peekPtr().tag();
}
...
void MarkStackIter::saveValueArray(NativeObject* obj, uintptr_t index, HeapSlot::Kind kind) {
  MOZ_ASSERT(peekTag() == MarkStack::ValueArrayTag);
...
void gc::PushArena(GCMarker* gcmarker, Arena* arena) {
  DispatchTraceKindTyped(PushArenaFunctor(), MapAllocToTraceKind(arena->getAllocKind()), gcmarker, arena);
}
...
size_t GCMarker::sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf, const AutoLockForExclusiveAccess& lock) const {
...
#ifdef DEBUG
Zone* GCMarker::stackContainsCrossZonePointerTo(const Cell* target) const {
  ...
  for (MarkStackIter iter(stack); !iter.done(); iter.next()) {
    ...
    if (source->is<ProxyObject>()) {
      ...
      MOZ_ASSERT_IF(!IsCrossCompartmentWrapper(source), IsObjectValueInCompartment(value, source->compartment()));
...
template <typename T>
void js::gc::StoreBuffer::MonoTypeBuffer<T>::trace(StoreBuffer* owner, TenuringTracer& mover) {
...
namespace js {
namespace gc {
template void StoreBuffer::MonoTypeBuffer<StoreBuffer::ValueEdge>::trace(StoreBuffer*, TenuringTracer&);
...
void js::gc::StoreBuffer::SlotsEdge::trace(TenuringTracer& mover) const {
  ...
  if (kind() == ElementKind) {
    ...
    mover.traceSlots(static_cast<HeapSlot*>(obj->getDenseElements() + clampedStart)->unsafeUnbarrieredForTracing(),
...
static inline void TraceWholeCell(TenuringTracer& mover, jit::JitCode* jitcode) {
  jitcode->traceChildren(&mover);
}

template <typename T>
static void TraceBufferedCells(TenuringTracer& mover, Arena* arena, ArenaCellSet* cells) {
...
void js::gc::StoreBuffer::traceWholeCells(TenuringTracer& mover) {
...
void js::TenuringTracer::traceObject(JSObject* obj) {
  NativeObject* nobj = CallTraceHook(TenuringFunctor(), this, obj, CheckGeneration::NoChecks, *this);
  if (!nobj)
    return;

  // Note: the contents of copy on write elements pointers are filled in
  // during parsing and cannot contain nursery pointers.
  if (!nobj->hasEmptyElements() && !nobj->denseElementsAreCopyOnWrite() && ObjectDenseElementsMayBeMarkable(nobj)) {
...
void js::TenuringTracer::traceObjectSlots(NativeObject* nobj, uint32_t start, uint32_t length) {
  HeapSlot* fixedStart;
  HeapSlot* fixedEnd;
  HeapSlot* dynStart;
  HeapSlot* dynEnd;
...
inline void js::TenuringTracer::insertIntoObjectFixupList(RelocationOverlay* entry) {
  *objTail = entry;
...
JSObject* js::TenuringTracer::moveToTenuredSlow(JSObject* src) {
  ...
  if (op) {
    ...
    MOZ_ASSERT_IF(src->getClass()->hasFinalize(), CanNurseryAllocateFinalizedClass(src->getClass()));
  }
...
inline JSObject* js::TenuringTracer::movePlainObjectToTenured(PlainObject* src) {
...
size_t js::TenuringTracer::moveSlotsToTenured(NativeObject* dst, NativeObject* src) {
...
size_t js::TenuringTracer::moveElementsToTenured(NativeObject* dst, NativeObject* src, AllocKind dstKind) {
  ...
  if (src->is<ArrayObject>() && nslots <= GetGCKindSlots(dstKind)) {
    ...
    nursery().setElementsForwardingPointer(srcHeader, dst->getElementsHeader(), srcHeader->capacity);
  ...
  ObjectElements* dstHeader;
  {
    ...
    if (!dstHeader) {
      oomUnsafe.crash(sizeof(HeapSlot) * nslots, "Failed to allocate elements while tenuring.");
    }
  }
...
inline void js::TenuringTracer::insertIntoStringFixupList(RelocationOverlay* entry) {
  *stringTail = entry;
...
void js::Nursery::collectToFixedPoint(TenuringTracer& mover, TenureCountCache& tenureCounts) {
...
size_t js::TenuringTracer::moveStringToTenured(JSString* dst, JSString* src, AllocKind dstKind) {
...
#undef IS_SAME_TYPE_OR
  ...
  MOZ_ASSERT_IF(!ThingIsPermanentAtomOrWellKnownSymbol(*thingp),
                CurrentThreadCanAccessRuntime(rt) || CurrentThreadCanAccessZone((*thingp)->zoneFromAnyThread()) ||
...
template <typename T>
static bool IsMarkedInternal(JSRuntime* rt, T** thingp) {
...
template <>
/* static */ bool IsMarkedInternal(JSRuntime* rt, JSObject** thingp) {
  if (IsOwnedByOtherRuntime(rt, *thingp))
    return true;
...
template <typename T>
static bool IsAboutToBeFinalizedInternal(T** thingp) {
  CheckIsMarkedThing(thingp);
  T* thing = *thingp;
  ...
  if (IsInsideNursery(thing)) {
    return JS::CurrentThreadIsHeapMinorCollecting() && !Nursery::getForwardedPointer(reinterpret_cast<Cell**>(thingp));
  }
...
template <typename T>
bool IsMarkedUnbarriered(JSRuntime* rt, T* thingp) {
  return IsMarkedInternal(rt, ConvertToBase(thingp));
}

template <typename T>
bool IsMarked(JSRuntime* rt, WriteBarrieredBase<T>* thingp) {
...
template <typename T>
bool IsAboutToBeFinalized(WriteBarrieredBase<T>* thingp) {
  return IsAboutToBeFinalizedInternal(ConvertToBase(thingp->unsafeUnbarrieredForTracing()));
}

template <typename T>
bool IsAboutToBeFinalized(ReadBarrieredBase<T>* thingp) {
  return IsAboutToBeFinalizedInternal(ConvertToBase(thingp->unsafeUnbarrieredForTracing()));
}
...
struct AssertNonGrayTracer : public JS::CallbackTracer {
  ...
  void onChild(const JS::GCCellPtr& thing) override { MOZ_ASSERT(!thing.asCell()->isMarkedGray()); }
};
#endif
...
class UnmarkGrayTracer : public JS::CallbackTracer {
 public:
  ...
  explicit UnmarkGrayTracer(JSRuntime* rt)
      : JS::CallbackTracer(rt, DoNotTraceWeakMaps), unmarkedAny(false), oom(false), stack(rt->gc.unmarkGrayStack) {}
