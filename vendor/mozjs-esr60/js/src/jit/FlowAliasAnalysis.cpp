...
class GraphStoreInfo : public TempObject {
  ...
  // Vector with pointer to BlockStoreInfo at the end of the block for every
  // block. Only keeping the info during iteration if needed, else contains
  // nullptr.
...
static bool LoadAliasesStore(MDefinition* load, MDefinition* store) {
  ...
  MDefinition::AliasType mightAlias =
      AliasAnalysisShared::genericMightAlias(load, store);
...
static void DumpStoreList(BlockStoreInfo& stores) {
  ...
  for (size_t i = 0; i < stores.length(); i++) {
    ...
    MOZ_ASSERT(stores[i]->isControlInstruction() ||
               stores[i]->getAliasSet().isStore() || stores[i]->id() == 0);
...
static void DumpLoadOutcome(MDefinition* load, MDefinitionVector& stores,
                            bool defer) {
#ifdef JS_JITSPEW
  // Spew what we did.
  if (JitSpewEnabled(JitSpew_Alias)) {
...
static void DumpLoopInvariant(MDefinition* load, MBasicBlock* loopheader,
                              bool loopinvariant,
  ...
  if (JitSpewEnabled(JitSpew_Alias)) {
    ...
    if (!loopinvariant) {
      ...
      print.printf(
          "   Determine loop invariant to loop %d. Dependendy is now: ",
          loopheader->id());
...
//         encountering a control instruction (indicates loop) it solely depends
//         on we defer until the loop has been examined.
//
...
bool FlowAliasAnalysis::processStore(BlockStoreInfo& blockInfo,
                                     MDefinition* store) {
...
bool FlowAliasAnalysis::processLoad(BlockStoreInfo& blockInfo,
                                    MDefinition* load) {
  DumpLoad(load);
...
bool FlowAliasAnalysis::processDeferredLoads(LoopInfo* info) {
  ...
  for (size_t i = 0; i < info->loopinvariant().length(); i++) {
    ...
    DumpLoopInvariant(load, info->loopHeader(),
...
bool FlowAliasAnalysis::improveDependency(MDefinition* load,
                                          MDefinitionVector& inputStores,
...
bool FlowAliasAnalysis::improveNonAliasedStores(MDefinition* load,
                                                MDefinitionVector& inputStores,
                                                MDefinitionVector& outputStores,
                                                bool* improved,
...
bool FlowAliasAnalysis::improveLoopDependency(MDefinition* load,
                                              MDefinitionVector& inputStores,
                                              MDefinitionVector& outputStores) {
  outputStores.clear();
  if (!outputStores.appendAll(inputStores)) return false;
...
bool FlowAliasAnalysis::improveStoresInFinishedLoops(MDefinition* load,
                                                     MDefinitionVector& stores,
                                                     bool* improved) {
  for (size_t i = 0; i < stores.length(); i++) {
...
void FlowAliasAnalysis::saveLoadDependency(MDefinition* load,
                                           MDefinitionVector& dependencies) {
...
bool FlowAliasAnalysis::saveStoreDependency(MDefinition* ins,
                                            BlockStoreInfo& prevStores) {
  // To form a store dependency chain, we store the previous last dependencies
  // in the current store.

  StoreDependency* dependency =
      new (alloc().fallible()) StoreDependency(alloc());
