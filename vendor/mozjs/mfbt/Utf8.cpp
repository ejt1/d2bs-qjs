...
mozilla::Tuple<size_t, size_t> mozilla::ConvertUtf16toUtf8Partial(
  ...
  const char16_t* srcLimit = srcPtr + srcLen;
  char* dstOrigPtr = aDest.Elements();
  char* dstPtr = dstOrigPtr;
  size_t dstLen = aDest.Length();
  const char* dstLimit = dstPtr + dstLen;

  WideCharToMultiByte(CP_UTF8, 0, reinterpret_cast<LPCWSTR>(srcPtr), srcLen, dstPtr, dstLen, nullptr, nullptr);
...
size_t mozilla::ConvertUtf8toUtf16(mozilla::Span<const char> aSource,
  ...
  MultiByteToWideChar(CP_UTF8, 0, srcPtr, srcLen, reinterpret_cast<LPWSTR>(dstPtr), dstLen);
