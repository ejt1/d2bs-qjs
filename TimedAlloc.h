...
class TimedAlloc {
 ...
 public:
  ...
  void DumpStats(FILE* f) const {
    ...
    double realalloctime = (alloctime / freq), realdealloctime = (dealloctime / freq), realctortime = (ctortime / freq), realdtortime = (dtortime / freq),
           allocperpsec = (allocs == 0 ? 0 : realalloctime * 1000 / allocs), deallocperpsec = (deallocs == 0 ? 0 : realdealloctime * 1000 / deallocs),
           ctorperobj = (ctors == 0 ? 0 : realctortime * 1000 / ctors), dtorperobj = (dtors == 0 ? 0 : realdtortime * 1000 / dtors);
...
inline bool operator==(const TimedAlloc<T>&, const TimedAlloc<U>&) {
  return true;
}
template <typename T, typename U>
inline bool operator!=(const TimedAlloc<T>&, const TimedAlloc<U>&) {
